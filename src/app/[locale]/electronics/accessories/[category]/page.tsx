...
const CATEGORIES: Record<string,{ label:string; hero:string; image:string; desc:string; count:string; brands:string[]; priceRanges:string[] }> = {
  'all-accessories': { label:'All Accessories', hero:IMGS.bose, image:IMGS.bose, desc:'Browse every electronic accessory from every brand.', count:'4,812', brands:['Bose','Sonos','GoPro','Logitech','Samsung','EcoFlow','Elgato'], priceRanges:['Any Price','0 – 500 MAD','500 – 1,500 MAD','1,500 – 3,500 MAD','3,500+ MAD'] },
  'headphones': { label:'Headphones', hero:IMGS.bose, image:IMGS.bose, desc:'Wireless, noise-cancelling, and studio headphones.', count:'1,240', brands:['Bose','Sonos','Logitech'], priceRanges:['Any Price','0 – 800 MAD','800 – 2,000 MAD','2,000 – 4,000 MAD','4,000+ MAD'] },
  'soundbars-speakers': { label:'Soundbars & Speakers', hero:IMGS.sonos, image:IMGS.sonos, desc:'Home audio soundbars and portable speakers.', count:'980', brands:['Sonos','Bose'], priceRanges:['Any Price','0 – 1,500 MAD','1,500 – 4,000 MAD','4,000 – 7,000 MAD','7,000+ MAD'] },
  'action-cameras': { label:'Action Cameras', hero:IMGS.gopro, image:IMGS.gopro, desc:'Action cams and accessories for every adventure.', count:'620', brands:['GoPro'], priceRanges:['Any Price','0 – 2,000 MAD','2,000 – 4,000 MAD','4,000+ MAD'] },
  'gaming-gear': { label:'Gaming Gear', hero:IMGS.logitech, image:IMGS.logitech, desc:'Keyboards, mice, and gaming accessories.', count:'1,110', brands:['Logitech','Elgato'], priceRanges:['Any Price','0 – 800 MAD','800 – 2,000 MAD','2,000 – 4,000 MAD','4,000+ MAD'] },
  'portable-power': { label:'Portable Power', hero:IMGS.ecoflow, image:IMGS.ecoflow, desc:'Power stations, banks, and solar chargers.', count:'340', brands:['EcoFlow'], priceRanges:['Any Price','0 – 2,000 MAD','2,000 – 6,000 MAD','6,000 – 10,000 MAD','10,000+ MAD'] },
  'chargers-cables': { label:'Chargers & Cables', hero:IMGS.samsung, image:IMGS.samsung, desc:'Fast chargers, cables, and power accessories.', count:'520', brands:['Samsung','Logitech'], priceRanges:['Any Price','0 – 300 MAD','300 – 800 MAD','800+ MAD'] },
}

const ALL_CATS = [
  { label:'All Accessories', slug:'all-accessories' },
  { label:'Headphones', slug:'headphones' },
  { label:'Soundbars & Speakers', slug:'soundbars-speakers' },
  { label:'Action Cameras', slug:'action-cameras' },
  { label:'Gaming Gear', slug:'gaming-gear' },
...
function makeListings(catSlug: string, count: number) {
  const cat = CATEGORIES[catSlug] || CATEGORIES['all-accessories']
  const titleMap: Record<string,string[]> = {
    'all-accessories': ['Bose Headphones 700','Sonos Beam Soundbar','GoPro HERO12 Black','Logitech G915 Keyboard'],
    'headphones': ['Bose QuietComfort Ultra','Sonos Ace Headphones','Logitech Zone Wireless'],
    'soundbars-speakers': ['Sonos Beam Gen 2','Bose Smart Soundbar 900','Sonos Move 2'],
    'action-cameras': ['GoPro HERO12 Black','GoPro Hero 11 Mini'],
    'gaming-gear': ['Logitech G915 TKL Keyboard','Elgato Stream Deck MK.2','Logitech MX Master 3S'],
    'portable-power': ['EcoFlow Delta 2','EcoFlow River 2 Max'],
    'chargers-cables': ['Samsung 65W GaN Charger','Belkin BoostCharge Pro'],
  }
  const titles = titleMap[catSlug] || titleMap['all-accessories']
...
export default function AccessoriesCategoryPage() {
  ...
  const catSlug = (params?.category as string) || 'all-accessories'
  const cat = CATEGORIES[catSlug] || CATEGORIES['all-accessories']
    ...
    <div style={{ fontFamily:"'Inter',sans-serif", backgroundColor:C.surface, minHeight:'100vh' }}>
      <section style={{ position:'relative', height:340, overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center' }}>
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center', padding:'0 24px', maxWidth:760, width:'100%' }}>
          <p style={{ fontSize:11, ...UB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.15em', marginBottom:10 }}>ELECTRONICS › ACCESSORIES</p>
      ...
      <div style={{ maxWidth:1440, margin:'32px auto 0', padding:'0 40px 80px' }}>
        <nav style={{ display:'flex', alignItems:'center', gap:6, fontSize:11, fontWeight:700, color:C.muted, textTransform:'uppercase' as const, letterSpacing:'0.06em', marginBottom:16 }}>
          ...
          <Link href={`/${locale}/electronics/accessories`} style={{ color:C.muted, textDecoration:'none' }}>Accessories</Link><span>›</span>
        ...
        <div style={{ display:'flex', gap:8, margin:'20px 0', flexWrap:'wrap' as const }}>
          {ALL_CATS.map(c=>(
            <Link key={c.slug} href={`/${locale}/electronics/accessories/${c.slug}`}
            ...
            {cat.brands.map(brand=>(
              <button key={brand} onClick={()=>setActiveBrand(activeBrand===brand?'':brand)}
                ...
                <div style={{ position:'relative', aspectRatio:'1/1', overflow:'hidden' }}>
                  <img src={IMGS[brand.toLowerCase() as keyof typeof IMGS] || cat.image} alt={brand} style={{ width:'100%', height:'100%', objectFit:'cover', transition:'transform 0.4s', transform:activeBrand===brand?'scale(1.06)':'scale(1)' }} />
        ...
        <section style={{ marginBottom:40 }}>
          <h3 style={{ ...UB, fontSize:16, color:C.ink, textTransform:'uppercase' as const, letterSpacing:'0.06em', marginBottom:16 }}>Explore Other Accessory Types</h3>
        ...
        <div style={{ textAlign:'center' as const }}>
          <Link href={`/${locale}/electronics/accessories`}
