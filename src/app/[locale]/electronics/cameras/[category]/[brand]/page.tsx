...
const BRAND_DATA: Record<string,{ label:string; hero:string; desc:string; models:string[]; priceRanges:string[] }> = {
  'sony': {
    label:'Sony Cameras',
    hero:'https://images.pexels.com/photos/1261731/pexels-photo-1261731.jpeg?auto=compress&w=1600',
    desc:'Sony Alpha, ZV and Cinema Line cameras in Morocco',
    models:['Alpha 7R V','Alpha 7 IV','Alpha 7C II','Alpha 7S III','ZV-E1','ZV-1 II','FX3','FX6','A6700','A6400'],
  ...
  'canon': {
    ...
    desc:'Canon EOS R, EOS and PowerShot cameras in Morocco',
    models:['EOS R6 Mark II','EOS R5','EOS R8','EOS R50','EOS R100','EOS 90D','PowerShot V10','EOS R3'],
    priceRanges:['Any Price','0–2500 MAD','2500–7000 MAD','7000–16000 MAD','16000+ MAD'],
  },
  'nikon': {
    label:'Nikon Cameras',
    hero:'https://images.pexels.com/photos/1649771/pexels-photo-1649771.jpeg?auto=compress&w=1600',
    desc:'Nikon Z series mirrorless and DSLR cameras in Morocco',
    models:['Z8','Z6 III','Z5 II','Z50 II','Z30','Zf','D850','D7500','Z9','Coolpix P1000'],
    priceRanges:['Any Price','0–2500 MAD','2500–7000 MAD','7000–16000 MAD','16000+ MAD'],
  },
  'fujifilm': {
    label:'Fujifilm Cameras',
    hero:'https://images.pexels.com/photos/1714207/pexels-photo-1714207.jpeg?auto=compress&w=1600',
    desc:'Fujifilm X-T, X-S, GFX and Instax cameras in Morocco',
    models:['X-T5','X-T50','X-S20','X100VI','GFX 100S II','X-E4','X-Pro3','Instax Mini 99','X-H2S'],
    priceRanges:['Any Price','0–2000 MAD','2000–6000 MAD','6000–14000 MAD','14000+ MAD'],
  },
  'panasonic': {
    label:'Panasonic Cameras',
    hero:'https://images.pexels.com/photos/3394650/pexels-photo-3394650.jpeg?auto=compress&w=1600',
    desc:'Panasonic Lumix S, G and BGH cinema cameras in Morocco',
    models:['Lumix S5 II','Lumix S5 IIX','Lumix G9 II','Lumix GH6','Lumix S1H','BGH1','Lumix S9','Lumix GH5 II'],
    priceRanges:['Any Price','0–2500 MAD','2500–7000 MAD','7000–15000 MAD','15000+ MAD'],
  },
  'dji': {
    label:'DJI Cameras',
    hero:'https://images.pexels.com/photos/1261731/pexels-photo-1261731.jpeg?auto=compress&w=1600',
    desc:'DJI drones, action cameras and gimbals in Morocco',
    models:['DJI Air 3S','DJI Mini 4 Pro','DJI Avata 2','Osmo Action 5 Pro','Osmo Pocket 3','DJI Neo','DJI RS 4','Mavic 3 Pro'],
    priceRanges:['Any Price','0–2000 MAD','2000–6000 MAD','6000–14000 MAD','14000+ MAD'],
  },
  'gopro': {
    label:'GoPro Cameras',
    hero:'https://images.pexels.com/photos/1649771/pexels-photo-1649771.jpeg?auto=compress&w=1600',
    desc:'GoPro HERO action cameras and accessories in Morocco',
    models:['HERO13 Black','HERO12 Black','HERO11 Black Mini','MAX 360','HERO11 Black','HERO10 Black','Volta Battery Grip','Enduro Battery'],
    priceRanges:['Any Price','0–1500 MAD','1500–4000 MAD','4000–8000 MAD','8000+ MAD'],
  },
}

const LOCS = ['Rabat','Casablanca','Marrakech','Tangier','Agadir','Fès','Meknès','Kenitra']
const IMGS = [
  'https://images.pexels.com/photos/1261731/pexels-photo-1261731.jpeg?auto=compress&w=400',
      ...
      <div style={{ padding:'14px 16px' }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          <a href="https://wa.me/212600000000" target="_blank" rel="noopener noreferrer"
            style={{ flex:1, backgroundColor:'#25D366', color:'white', border:'none', padding:'9px', borderRadius:12, fontSize:'10px', ...CB, textTransform:'uppercase' as const, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', textDecoration:'none' }}>WhatsApp</a>
        </div>
      </div>
    </article>
  )
...
export default function CamerasBrandPage() {
  const params    = useParams()
  const locale    = (params?.locale   as string) || 'en'
  const category  = (params?.category as string) || 'all-cameras'
  const brandSlug = (params?.brand    as string) || 'sony'
  const brandData = BRAND_DATA[brandSlug] || BRAND_DATA['sony']
    ...
    <div style={{ ...UB, backgroundColor:C.surface, color:C.ink, minHeight:'100vh' }}>
      <section style={{ position:'relative', height:360, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center' as const, maxWidth:760, padding:'0 24px', width:'100%' }}>
          <p style={{ fontSize:11, ...UB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.2em', marginBottom:12 }}>CAMERAS › {brandData.label.toUpperCase()}</p>
      ...
      <main style={{ maxWidth:1280, margin:'0 auto', padding:'32px 24px 80px' }}>
        <nav style={{ display:'flex', alignItems:'center', gap:6, fontSize:10, ...UB, color:C.muted, textTransform:'uppercase' as const, letterSpacing:'0.12em', marginBottom:12 }}>
          {[
            ...
            {label:'Cameras',href:`/${locale}/electronics/cameras`},
            {label:'All Cameras',href:`/${locale}/electronics/cameras/${category}`},
        ...
        <div style={{ textAlign:'center' as const }}>
          <Link href={`/${locale}/electronics/cameras/${category}`}
