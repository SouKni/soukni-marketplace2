...
const IMGS = {
  ...
  alpine:'https://images.pexels.com/photos/1649771/pexels-photo-1649771.jpeg?auto=compress&w=600',
  jvc:'https://images.pexels.com/photos/1229861/pexels-photo-1229861.jpeg?auto=compress&w=600',
  sony:'https://images.pexels.com/photos/1229861/pexels-photo-1229861.jpeg?auto=compress&w=600',
  baseus:'https://images.pexels.com/photos/4526407/pexels-photo-4526407.jpeg?auto=compress&w=600',
}
...
const CATEGORIES: Record<string,{ label:string; hero:string; image:string; desc:string; count:string; brands:string[]; priceRanges:string[] }> = {
  'all-car-electronics': { label:'All Car Electronics', hero:IMGS.screen, image:IMGS.screen, desc:'Browse every car electronics category from every brand.', count:'2,415', brands:['Pioneer','Sony','Kenwood','JVC','Alpine','Garmin','Baseus'], priceRanges:['Any Price','0 – 500 MAD','500 – 2,000 MAD','2,000 – 6,000 MAD','6,000+ MAD'] },
  'android-screens': { label:'Android Screens', hero:IMGS.screen, image:IMGS.screen, desc:'Android head units and touchscreen replacements.', count:'480', brands:['Sony','Pioneer','JVC'], priceRanges:['Any Price','0 – 2,000 MAD','2,000 – 5,000 MAD','5,000+ MAD'] },
  'dash-cams': { label:'Dash Cams', hero:IMGS.dashcam, image:IMGS.dashcam, desc:'Front and dual-channel dash cameras.', count:'620', brands:['Garmin','Sony'], priceRanges:['Any Price','0 – 800 MAD','800 – 2,000 MAD','2,000 – 4,000 MAD','4,000+ MAD'] },
  'audio-speakers': { label:'Audio & Speakers', hero:IMGS.jbl, image:IMGS.jbl, desc:'Car speakers, subwoofers, and amplifiers.', count:'740', brands:['JBL','Alpine','Kenwood','Pioneer'], priceRanges:['Any Price','0 – 800 MAD','800 – 2,000 MAD','2,000 – 5,000 MAD','5,000+ MAD'] },
  'navigation': { label:'Navigation', hero:IMGS.garmin, image:IMGS.garmin, desc:'GPS navigation systems and off-road trackers.', count:'310', brands:['Garmin'], priceRanges:['Any Price','0 – 1,000 MAD','1,000 – 4,000 MAD','4,000 – 8,000 MAD','8,000+ MAD'] },
  'security-systems': { label:'Security Systems', hero:IMGS.dashcam, image:IMGS.dashcam, desc:'GPS trackers, alarms, and vehicle security.', count:'265', brands:['Garmin','Baseus'], priceRanges:['Any Price','0 – 500 MAD','500 – 1,500 MAD','1,500+ MAD'] },
}

const ALL_CATS = [
  { label:'All Car Electronics', slug:'all-car-electronics' },
  { label:'Android Screens', slug:'android-screens' },
  { label:'Dash Cams', slug:'dash-cams' },
  { label:'Audio & Speakers', slug:'audio-speakers' },
  { label:'Navigation', slug:'navigation' },
  { label:'Security Systems', slug:'security-systems' },
]

const BRAND_IMG: Record<string,string> = { Pioneer:IMGS.pioneer, Sony:IMGS.sony, Kenwood:IMGS.kenwood, JVC:IMGS.jvc, Alpine:IMGS.alpine, Garmin:IMGS.garmin, Baseus:IMGS.baseus, JBL:IMGS.jbl }
...
function makeListings(catSlug: string, count: number) {
  const cat = CATEGORIES[catSlug] || CATEGORIES['all-car-electronics']
  const titleMap: Record<string,string[]> = {
    'all-car-electronics': ['Pioneer XDJ-XZ Audio Hub','Android 13 12.1" Screen','4K Dual Dash Cam','Garmin Overlander GPS'],
    'android-screens': ['Android 13 12.1" Screen For Toyota','Sony Xperia Head Unit','JVC Android Display'],
    'dash-cams': ['4K Dual Dash Cam Night Vision','Garmin Dash Cam Mini 2','Dual-channel 4K Dash Cam'],
    'audio-speakers': ['JBL Stage 3 Car Speakers','Alpine S-W10D4 Subwoofer','Kenwood DMX-7709S Receiver'],
    'navigation': ['Garmin Overlander GPS','Garmin DriveSmart 65'],
    'security-systems': ['GPS Real-time Tracker 4G','OBD2 Diagnostic Scanner'],
  }
  const titles = titleMap[catSlug] || titleMap['all-car-electronics']
...
export default function CarElectronicsCategoryPage() {
  const params = useParams()
  const locale = (params?.locale as string) || 'en'
  const catSlug = (params?.category as string) || 'all-car-electronics'
  const cat = CATEGORIES[catSlug] || CATEGORIES['all-car-electronics']
    ...
    <div style={{ fontFamily:"'Inter',sans-serif", backgroundColor:C.surface, minHeight:'100vh' }}>
      <section style={{ position:'relative', height:340, overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center' }}>
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center', padding:'0 24px', maxWidth:760, width:'100%' }}>
          <p style={{ fontSize:11, ...UB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.15em', marginBottom:10 }}>ELECTRONICS › CAR ELECTRONICS</p>
      ...
      <div style={{ maxWidth:1440, margin:'32px auto 0', padding:'0 40px 80px' }}>
        ...
        <div style={{ display:'flex', gap:8, margin:'20px 0', flexWrap:'wrap' as const }}>
          {ALL_CATS.map(c=>(
            <Link key={c.slug} href={`/${locale}/electronics/car-electronics/${c.slug}`}
        ...
        <section style={{ marginBottom:40 }}>
          <h3 style={{ ...UB, fontSize:16, color:C.ink, textTransform:'uppercase' as const, letterSpacing:'0.06em', marginBottom:16 }}>Explore Other Car Tech Categories</h3>
        ...
        <div style={{ textAlign:'center' as const }}>
          <Link href={`/${locale}/electronics/car-electronics`}
          ...
          >Back to All Car Electronics</Link>
        </div>
      </div>
    </div>
  )
