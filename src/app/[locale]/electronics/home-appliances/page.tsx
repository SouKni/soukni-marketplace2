...
function Badge({ type }: { type: string }) {
  ...
  if (type === 'verified') return (
    <span style={{ backgroundColor: '#006b5f', color: 'white', fontSize: '9px', fontWeight: 800, padding: '3px 9px', borderRadius: '100px', letterSpacing: '0.08em', textTransform: 'uppercase' as const, fontFamily: 'Inter,sans-serif' }}>
      Verified
    </span>
  )
  return (
    ...
    >
      ...
      <div style={{ padding: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
          ...
          <span style={{ fontSize: '16px', fontWeight: 800, color: '#006b5f', fontFamily: 'Inter,sans-serif' }}>{formatPrice(item.price)}</span>
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button style={{ flex: 1, padding: '10px', borderRadius: '100px', border: '1px solid #006b5f', color: '#006b5f', backgroundColor: 'transparent', fontWeight: 700, fontSize: '13px', cursor: 'pointer', fontFamily: 'Inter,sans-serif', transition: 'background 0.15s' }}
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '20px' }}>
        ...
        <button style={{ width: '100%', backgroundColor: '#006b5f', color: 'white', border: 'none', padding: '12px', borderRadius: '100px', fontWeight: 700, fontSize: '14px', cursor: 'pointer', fontFamily: 'Inter,sans-serif', transition: 'filter 0.15s' }}
...
function SectionHeader({ title, href, locale }: { title: string; href: string; locale: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
      ...
      <Link href={`/${locale}/${href}`} style={{ color: '#006b5f', fontWeight: 700, fontSize: '13px', textDecoration: 'none', transition: 'opacity 0.15s' }}
...
function DiamondToggle({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) {
  return (
    ...
    >
      ...
      <div style={{ width: '44px', height: '24px', borderRadius: '100px', backgroundColor: enabled ? '#006b5f' : '#bacac5', position: 'relative', transition: 'background 0.25s', flexShrink: 0 }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 24px' }}>
          ...
          <div style={{ width: '100%', maxWidth: '680px', backgroundColor: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '100px', padding: '6px' }}>
            <div style={{ display: 'flex', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '100px', padding: '10px 20px' }}>
              ...
              />
              <button style={{ backgroundColor: '#006b5f', color: 'white', border: 'none', padding: '10px 28px', borderRadius: '100px', fontWeight: 700, fontSize: '13px', cursor: 'pointer', flexShrink: 0, transition: 'filter 0.15s' }}
                onMouseEnter={e => e.currentTarget.style.filter = 'brightness(1.1)'}
                onMouseLeave={e => e.currentTarget.style.filter = 'brightness(1)'}
              >Search</button>
            </div>
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 40px' }}>
        ...
        <section style={{ marginBottom: '40px' }}>
          <div style={{ backgroundColor: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.4)', borderRadius: '100px', boxShadow: '0 8px 32px rgba(0,0,0,0.06)', padding: '6px', display: 'flex', alignItems: 'center' }}>
            {[
              { label: 'City', value: 'Rabat', icon: <ChevronDown size={18} color="#006b5f" /> },
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    {field.isInput ? (
                      ...
                      <span style={{ fontSize: '15px', fontWeight: field.isActive ? 700 : 400, color: field.isActive ? '#006b5f' : '#161d1b', fontFamily: 'Inter,sans-serif' }}>{field.value}</span>
                    )}
                    {field.icon && <span style={{ flexShrink: 0 }}>{field.icon}</span>}
                    {field.isActive && <span style={{ fontSize: '18px', color: '#006b5f' }}>🎚</span>}
                  </div>
                </div>
        ...
        <section style={{ marginBottom: '32px' }}>
          ...
          <nav style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: '#6b7a76', marginBottom: '16px', fontFamily: 'Inter,sans-serif' }}>
            <Link href={`/${locale}`} style={{ color: '#6b7a76', textDecoration: 'none', transition: 'color 0.15s' }} onMouseEnter={e => e.currentTarget.style.color = '#006b5f'} onMouseLeave={e => e.currentTarget.style.color = '#6b7a76'}>Home</Link>
            <span style={{ color: '#bacac5' }}>›</span>
            <Link href={`/${locale}/electronics`} style={{ color: '#6b7a76', textDecoration: 'none', transition: 'color 0.15s' }} onMouseEnter={e => e.currentTarget.style.color = '#006b5f'} onMouseLeave={e => e.currentTarget.style.color = '#6b7a76'}>Electronics</Link>
            <span style={{ color: '#bacac5' }}>›</span>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {['All Sellers', 'SouKni Members', 'SouKni Pro'].map(tab => (
              <button key={tab} onClick={() => setActiveSeller(tab)}
                style={{ padding: '8px 20px', borderRadius: '10px', fontSize: '13px', fontWeight: 700, border: activeSeller === tab ? 'none' : '1px solid rgba(186,202,197,0.6)', backgroundColor: activeSeller === tab ? '#006b5f' : 'transparent', color: activeSeller === tab ? 'white' : '#3c4a46', cursor: 'pointer', fontFamily: 'Inter,sans-serif', transition: 'all 0.15s' }}
              >{tab}</button>
            ))}
          </div>
        </section>
          ...
          <div style={{ position: 'relative', borderRadius: '40px', overflow: 'hidden', height: '300px', cursor: 'pointer' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', padding: '48px' }}>
              <div style={{ maxWidth: '480px', position: 'relative', zIndex: 1 }}>
                ...
                <button style={{ backgroundColor: 'white', color: '#006b5f', border: 'none', padding: '12px 28px', borderRadius: '100px', fontWeight: 700, fontSize: '14px', cursor: 'pointer', fontFamily: 'Inter,sans-serif', transition: 'all 0.2s' }}
                  onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#2dd4bf'; e.currentTarget.style.color = 'white' }}
                  onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'white'; e.currentTarget.style.color = '#006b5f' }}
        ...
        <section style={{ marginBottom: '80px' }}>
          <div style={{ position: 'relative', borderRadius: '40px', overflow: 'hidden', padding: '56px', background: 'linear-gradient(135deg, #006b5f 0%, #2dd4bf 100%)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '40px', boxShadow: '0 20px 60px rgba(0,107,95,0.2)' }}>
            <div style={{ maxWidth: '560px', position: 'relative', zIndex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' as const }}>
                <button style={{ backgroundColor: 'white', color: '#006b5f', border: 'none', padding: '14px 36px', borderRadius: '100px', fontWeight: 800, fontSize: '15px', cursor: 'pointer', fontFamily: 'Inter,sans-serif', boxShadow: '0 8px 24px rgba(0,0,0,0.15)', transition: 'transform 0.2s' }}
      ...
      <footer style={{ backgroundColor: '#7a7a7a', color: 'white', paddingTop: '64px', paddingBottom: '32px', fontFamily: 'Inter,sans-serif' }}>
        <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 40px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', gap: '48px', marginBottom: '48px', paddingBottom: '48px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
            {/* Brand */}
            <div>
              <h1 style={{ fontSize: '36px', fontWeight: 900, color: 'white', letterSpacing: '-0.03em', marginBottom: '16px' }}>SouKni</h1>
              <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.7, maxWidth: '280px', marginBottom: '24px' }}>
                Morocco's leading marketplace for premium appliances and luxury goods. Trusted by thousands in Rabat for discovery and effortless trading.
              </p>
              <div style={{ display: 'flex', gap: '12px' }}>
                {['🌐', '📤', '👥'].map((icon, i) => (
                  <a key={i} href="#" style={{ width: '40px', height: '40px', borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', textDecoration: 'none', transition: 'background 0.15s' }}
            ...
            {/* Directory */}
            <div>
              <h5 style={{ color: 'white', fontWeight: 700, marginBottom: '20px', fontSize: '14px' }}>Directory</h5>
              {['About Us', 'Terms of Service', 'Privacy Policy', 'Cookie Settings'].map(link => (
            ...
            {/* Support */}
            <div>
              <h5 style={{ color: 'white', fontWeight: 700, marginBottom: '20px', fontSize: '14px' }}>Support</h5>
              {['Help Center', 'Trust & Safety', 'Contact Support', 'Diamond Membership'].map(link => (
                <a key={link} href="#" style={{ display: 'block', fontSize: '13px', color: 'rgba(255,255,255,0.65)', textDecoration: 'none', marginBottom: '12px', transition: 'color 0.15s' }}
                  onMouseEnter={e => e.currentTarget.style.color = '#2dd4bf'}
                  onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.65)'}
                >{link}</a>
            ...
            {/* App Downloads */}
            <div>
              <h5 style={{ color: 'white', fontWeight: 700, marginBottom: '20px', fontSize: '14px' }}>Download the App</h5>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {[{ icon: '📱', store: 'App Store', sub: 'Download on the' }, { icon: '▶', store: 'Google Play', sub: 'Get it on' }].map(({ icon, store, sub }) => (
                  <a key={store} href="#" style={{ display: 'flex', alignItems: 'center', gap: '12px', backgroundColor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', padding: '12px 16px', borderRadius: '12px', textDecoration: 'none', transition: 'background 0.15s' }}
                    onMouseEnter={e => e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.12)'}
                    onMouseLeave={e => e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.06)'}
                  >
                    <span style={{ fontSize: '24px' }}>{icon}</span>
                    <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.4 }}>{sub}<br /><b style={{ fontSize: '13px', color: 'white' }}>{store}</b></span>
                  </a>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: '12px' }}>
            <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>© 2026 SouKni Rabat. All rights reserved. Premium Marketplace for Moroccan Appliances.</p>
