...
const I = {
  hero:   'https://images.pexels.com/photos/18105/pexels-photo.jpg?auto=compress&w=1600',
  g1:     'https://images.pexels.com/photos/18105/pexels-photo.jpg?auto=compress&w=400',
  g2:     'https://images.pexels.com/photos/205421/pexels-photo-205421.jpeg?auto=compress&w=400',
  g3:     'https://images.pexels.com/photos/1229861/pexels-photo-1229861.jpeg?auto=compress&w=400',
  g4:     'https://images.pexels.com/photos/574071/pexels-photo-574071.jpeg?auto=compress&w=400',
  g5:     'https://images.pexels.com/photos/18105/pexels-photo.jpg?auto=compress&w=400',
  bento1: 'https://images.pexels.com/photos/18105/pexels-photo.jpg?auto=compress&w=900',
  bento2: 'https://images.pexels.com/photos/205421/pexels-photo-205421.jpeg?auto=compress&w=600',
  bento3: 'https://images.pexels.com/photos/1229861/pexels-photo-1229861.jpeg?auto=compress&w=600',
  bento4: 'https://images.pexels.com/photos/574071/pexels-photo-574071.jpeg?auto=compress&w=600',
...
const featuredItems = [
  { brand:'Apple',     title:'MacBook Pro M4 Max 16" 64GB',       price:45000, location:'Rabat, Agdal',    img:I.g1, badges:['featured','diamond'],   condition:'New'      },
  { brand:'Razer',     title:'Blade 16 (2024) RTX 4090 OLED',     price:38000, location:'Rabat, Souissi',  img:I.g2, badges:['featured','certified'], condition:'New'      },
  { brand:'ASUS',      title:'ROG Zephyrus G16 Core Ultra 9',     price:28500, location:'Rabat, Hassan',   img:I.g3, badges:['featured','certified'], condition:'Like New' },
  { brand:'Dell',      title:'XPS 15 (9530) i9 32GB 1TB',         price:24000, location:'Rabat, Hay Riad', img:I.g4, badges:['featured','new'],       condition:'New'      },
]

function makeGrid(count: number) {
  const brands = ['Apple','HP','Lenovo','Dell','ASUS','MSI','Razer','Microsoft','Acer','Samsung']
  const titles = ['Spectre x360 14" OLED','ThinkPad X1 Carbon Gen 12','Surface Laptop 7','Titan 18 HX Core i9','MacBook Pro 14" M3 Pro','Envy 16" RTX 4060','TUF Gaming F15','Legion Pro 5i','Swift Go 14','Galaxy Book4 Ultra']
...
const CATS = [
  { label:'All Laptops', slug:'all-laptops' },
...
export default function LaptopsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const displayFeatured = displayFeaturedAll.filter((item: any) =>
    (!applied.keyword.trim() || item.title.toLowerCase().includes(applied.keyword.toLowerCase()) || item.brand.toLowerCase().includes(applied.keyword.toLowerCase())) &&
    (!applied.city.trim() || (item.location || '').toLowerCase().includes(applied.city.toLowerCase())) &&
    priceInRange(item.price, applied.price)
  )
  const displayGrid = displayGridAll.filter((item: any) =>
    (!applied.keyword.trim() || item.title.toLowerCase().includes(applied.keyword.toLowerCase()) || item.brand.toLowerCase().includes(applied.keyword.toLowerCase())) &&
    priceInRange(item.price, applied.price)
  )
  const PAGE_SIZE = 16
  const totalPages = Math.max(1, Math.ceil(displayGrid.length / PAGE_SIZE))
  const clampedPage = Math.min(page, totalPages)
  const paginatedGrid = displayGrid.slice((clampedPage - 1) * PAGE_SIZE, clampedPage * PAGE_SIZE)
  ...
  return (
    <div style={{ ...UB, backgroundColor:C.surface, color:C.ink, minHeight:'100vh' }}>
      ...
      <section style={{ position:'relative', height:'520px', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        <img src={I.hero} alt="Laptops" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }} />
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center' as const, maxWidth:'960px', padding:'0 24px', width:'100%' }}>
          <h1 style={{ fontSize:'clamp(36px,6vw,64px)', ...UB, color:'white', marginBottom:'36px', lineHeight:1, textShadow:'0 4px 20px rgba(0,0,0,0.4)' }}>
            LAPTOPS.<br/><span style={{ color:C.mint }}>POWER TO GO IN RABAT.</span>
          </h1>
          <div style={{ maxWidth:'780px', margin:'0 auto', backgroundColor:'rgba(255,255,255,0.12)', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,0.22)', borderRadius:'100px', padding:'8px', display:'flex', alignItems:'center' }}>
            ...
            <div style={{ flex:2, padding:'0 28px', display:'flex', flexDirection:'column' as const, gap:'2px' }}>
              ...
              <input type="text" value={keyword} onChange={e=>setKeyword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&applySearch()} placeholder="Search brands, models, specs..."
        ...
        <div style={{ backgroundColor:'rgba(255,255,255,0.97)', backdropFilter:'blur(16px)', border:'1px solid rgba(107,122,118,0.12)', borderRadius:'100px', boxShadow:'0 12px 40px rgba(0,0,0,0.08)', display:'flex', alignItems:'stretch', height:'72px' }}>
          ...
          <div style={{ flex:1.8, padding:'0 22px', display:'flex', flexDirection:'column' as const, justifyContent:'center' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
              ...
              <input type="text" value={keyword} onChange={e=>setKeyword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&applySearch()} placeholder="e.g. MacBook, ThinkPad, RTX 4090..."
      ...
      <main style={{ maxWidth:'1280px', margin:'0 auto', padding:'32px 24px 80px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'16px', marginBottom:'24px', flexWrap:'wrap' as const }}>
          <div>
            <h2 style={{ fontSize:'clamp(20px,2.5vw,28px)', ...UB, color:C.ink, marginBottom:'4px' }}>New and Pre-Owned Laptops in Rabat</h2>
            <p style={{ fontSize:'14px', color:C.mint, ...CB }}>4,218 Ads in Rabat District</p>
          </div>
        ...
        <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' as const, marginBottom:'20px', alignItems:'center' }}>
          {CATS.map(cat=>(
            <Link key={cat.slug} href={`/${locale}/electronics/laptops/${cat.slug}`}
        ...
        {/* ══ 7. FEATURED PREMIUM LAPTOPS ══════════════════════ */}
        <section style={{ marginBottom:'48px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
            <div>
              <h3 style={{ fontSize:'clamp(18px,2.5vw,24px)', ...UB, color:C.ink, textTransform:'uppercase' as const, marginBottom:'4px' }}>Featured Premium Laptops</h3>
              <p style={{ fontSize:'14px', color:C.muted }}>Hand-picked machines from top brands</p>
            </div>
        ...
        <section style={{ marginBottom:'64px' }}>
          <div style={{ position:'relative', borderRadius:'40px', overflow:'hidden', height:'320px', display:'flex', alignItems:'center', boxShadow:'0 16px 48px rgba(0,0,0,0.15)', cursor:'pointer' }}>
            ...
            <div style={{ position:'relative', zIndex:1, padding:'0 64px', maxWidth:'560px' }}>
              ...
              <h2 style={{ fontSize:'clamp(26px,3.5vw,42px)', ...UB, color:'white', marginBottom:'18px', lineHeight:1.05 }}>Work from anywhere, drive anywhere.</h2>
              <p style={{ fontSize:'17px', color:'rgba(255,255,255,0.9)', marginBottom:'28px', lineHeight:1.55 }}>Premium vehicle rentals for mobile professionals across Morocco.</p>
        ...
        {/* ══ 9. EXCLUSIVE LAPTOPS COLLECTION BENTO ════════════ */}
        <section style={{ marginBottom:'64px' }}>
          <h3 style={{ fontSize:'clamp(20px,3vw,32px)', ...UB, color:C.ink, textTransform:'uppercase' as const, marginBottom:'32px' }}>Exclusive Laptops Collection</h3>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gridTemplateRows:'380px 380px', gap:'20px' }}>
            <div style={{ gridColumn:'1/3', gridRow:'1/3', backgroundColor:'white', borderRadius:'40px', overflow:'hidden', display:'flex', flexDirection:'column' as const, border:'1px solid rgba(107,122,118,0.1)' }}>
              <div style={{ flex:1, overflow:'hidden', position:'relative', minHeight:0 }}>
                <img src={I.bento1} alt="MacBook Pro" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
              ...
              <div style={{ padding:'24px 28px', flexShrink:0 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'6px', gap:'12px' }}>
                  <div>
                    ...
                    <h4 style={{ fontSize:'20px', ...CB, color:C.ink }}>MacBook Pro M4 Max 16" — Space Black</h4>
                  </div>
                  <span style={{ fontSize:'22px', ...CB, color:C.mint, flexShrink:0 }}>45,000 MAD</span>
                </div>
                <p style={{ fontSize:'13px', color:C.muted, ...CB, marginBottom:'16px' }}>64GB RAM · 2TB SSD · Brand new sealed · Rabat, Agdal</p>
                <div style={{ display:'flex', gap:'10px' }}>
                  <MessageSellerButton listingId={undefined} sellerId={undefined} style={{ flex:1, border:`2px solid rgba(107,122,118,0.2)`, color:C.ink, backgroundColor:'transparent', padding:'12px', borderRadius:'16px', fontSize:'11px', ...CB, textTransform:'uppercase' as const }}>Message</MessageSellerButton>
                  <WhatsAppButton phone={undefined} title='MacBook Pro M4 Max 16" — Space Black' style={{ flex:1, backgroundColor:C.mint, color:C.ink, padding:'12px', borderRadius:'16px', fontSize:'11px', ...CB, textTransform:'uppercase' as const }}>💬 WhatsApp</WhatsAppButton>
                </div>
              </div>
            </div>
            <div style={{ gridColumn:'3', gridRow:'1', backgroundColor:'white', borderRadius:'32px', overflow:'hidden', display:'flex', flexDirection:'column' as const, border:'1px solid rgba(107,122,118,0.1)' }}>
              <div style={{ flex:1, overflow:'hidden', position:'relative', minHeight:0 }}>
                <img src={I.bento2} alt="Razer" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
              ...
              <div style={{ padding:'16px 18px', flexShrink:0 }}>
                <p style={{ fontSize:'9px', ...CB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.1em', marginBottom:'2px' }}>RAZER</p>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'6px', marginBottom:'10px' }}>
                  <h4 style={{ fontSize:'13px', ...CB, color:C.ink }}>Blade 16 RTX 4090 OLED 240Hz</h4>
                  <span style={{ fontSize:'14px', ...CB, color:C.mint, flexShrink:0 }}>38,000 MAD</span>
                </div>
                <div style={{ display:'flex', gap:'6px' }}>
                  ...
                  <WhatsAppButton phone={undefined} title="Blade 16 RTX 4090 OLED 240Hz" style={{ flex:1, backgroundColor:C.mint, color:C.ink, padding:'8px', borderRadius:'12px', fontSize:'9px', ...CB }} />
                </div>
              </div>
            </div>
            <div style={{ gridColumn:'4', gridRow:'1', backgroundColor:'white', borderRadius:'32px', overflow:'hidden', display:'flex', flexDirection:'column' as const, border:'1px solid rgba(107,122,118,0.1)' }}>
              <div style={{ flex:1, overflow:'hidden', position:'relative', minHeight:0 }}>
                <img src={I.bento3} alt="ASUS" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                <div style={{ position:'absolute', top:'12px', left:'12px' }}>
              ...
              <div style={{ padding:'16px 18px', flexShrink:0 }}>
                <p style={{ fontSize:'9px', ...CB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.1em', marginBottom:'2px' }}>ASUS</p>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'6px', marginBottom:'10px' }}>
                  <h4 style={{ fontSize:'13px', ...CB, color:C.ink }}>ROG Zephyrus G16 Core Ultra 9</h4>
                  <span style={{ fontSize:'14px', ...CB, color:C.mint, flexShrink:0 }}>28,500 MAD</span>
                </div>
                <div style={{ display:'flex', gap:'6px' }}>
                  <MessageSellerButton listingId={undefined} sellerId={undefined} style={{ flex:1, border:`1px solid rgba(107,122,118,0.2)`, color:C.ink, backgroundColor:'transparent', padding:'8px', borderRadius:'12px', fontSize:'9px', ...CB }}>Message</MessageSellerButton>
                  <WhatsAppButton phone={undefined} title="ROG Zephyrus G16 Core Ultra 9" style={{ flex:1, backgroundColor:C.mint, color:C.ink, padding:'8px', borderRadius:'12px', fontSize:'9px', ...CB }} />
                </div>
              </div>
            </div>
            <div style={{ gridColumn:'3/5', gridRow:'2', backgroundColor:'white', borderRadius:'32px', overflow:'hidden', display:'flex', flexDirection:'row' as const, border:'1px solid rgba(107,122,118,0.1)' }}>
              <div style={{ width:'50%', overflow:'hidden', position:'relative' }}>
                <img src={I.bento4} alt="Dell" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                <div style={{ position:'absolute', top:'12px', left:'12px' }}>
              ...
              <div style={{ flex:1, padding:'28px 32px', display:'flex', flexDirection:'column' as const, justifyContent:'center' }}>
                <p style={{ fontSize:'11px', ...CB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.1em', marginBottom:'6px' }}>DELL</p>
                <h4 style={{ fontSize:'18px', ...CB, color:C.ink, marginBottom:'6px' }}>XPS 15 (9530) i9 32GB 1TB</h4>
                <p style={{ fontSize:'13px', color:C.muted, ...CB, marginBottom:'12px' }}>4K OLED Touch · Brand new · Full warranty</p>
                <span style={{ fontSize:'22px', ...CB, color:C.mint, marginBottom:'20px', display:'block' }}>24,000 MAD</span>
                <div style={{ display:'flex', gap:'10px' }}>
                  <MessageSellerButton listingId={undefined} sellerId={undefined} style={{ flex:1, border:`2px solid rgba(107,122,118,0.2)`, color:C.ink, backgroundColor:'transparent', padding:'12px', borderRadius:'16px', fontSize:'10px', ...CB }}>Message</MessageSellerButton>
                  <WhatsAppButton phone={undefined} title="XPS 15 (9530) i9 32GB 1TB" style={{ flex:1, backgroundColor:C.mint, color:C.ink, padding:'12px', borderRadius:'16px', fontSize:'10px', ...CB }}>💬 WhatsApp</WhatsAppButton>
                </div>
              </div>
            </div>
          </div>
        ...
        <section style={{ marginBottom:'48px' }}>
          ...
          {[paginatedGrid.slice(0,4),paginatedGrid.slice(4,8),paginatedGrid.slice(8,12),paginatedGrid.slice(12,16)].map((row,ri)=>(
        ...
        <section style={{ marginBottom:'64px' }}>
          <div style={{ backgroundColor:C.mint, borderRadius:'40px', padding:'56px 64px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:`0 16px 48px ${C.mint}30`, flexWrap:'wrap' as const, gap:'28px' }}>
            <div style={{ maxWidth:'520px' }}>
              ...
              <p style={{ fontSize:'17px', color:`${C.ink}b3`, lineHeight:1.6 }}>Priority placement, SouKni Certified status, and direct marketing tools. Sell your laptops 5x faster.</p>
            </div>
