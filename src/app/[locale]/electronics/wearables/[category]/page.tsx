...
export default function WearablesCategoryPage() {
  ...
  const PAGE_SIZE = 16
  const totalPages = Math.max(1, Math.ceil(allFilteredListings.length / PAGE_SIZE))
  const clampedPage = Math.min(page, totalPages)
  const listings = allFilteredListings.slice((clampedPage - 1) * PAGE_SIZE, clampedPage * PAGE_SIZE)
  ...
  return (
    <div style={{ ...UB, backgroundColor:C.surface, color:C.ink, minHeight:'100vh' }}>
      ...
      <main style={{ maxWidth:'1280px', margin:'0 auto', padding:'32px 24px 80px' }}>
        <Breadcrumb
          items={[{label:'Rabat',href:`/${locale}`},{label:'Electronics',href:`/${locale}/electronics`},{label:'Wearables',href:`/${locale}/electronics/wearables`},{label:catData.label,href:null}]}
          mutedColor={C.muted}
          inkColor={C.ink}
        />
        ...
        <section style={{ marginBottom:'48px' }}>
          <p style={{ fontSize:'13px', color:C.muted, ...CB, marginBottom:'20px' }}>Showing {listings.length} of {allFilteredListings.length} results</p>
