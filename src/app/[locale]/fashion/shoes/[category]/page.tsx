...
const CATEGORIES: Record<string,{
  ...
  'heels-pumps': {
    label:'Heels & Pumps',
    hero:'https://images.pexels.com/photos/336372/pexels-photo-336372.jpeg?auto=compress&w=1600',
    desc:'Discover the finest heels and pumps from the world\'s most prestigious maisons.',
    count:'1,248',
    brands:['Christian Louboutin','Manolo Blahnik','Jimmy Choo','YSL','Valentino','Gianvito Rossi'],
  ...
  'sneakers': {
    ...
    desc:'Premium and rare sneakers — from limited drops to iconic classics.',
    count:'2,104',
    brands:['Nike','Jordan','Adidas','New Balance','Asics','Golden Goose','Off-White','Balenciaga'],
    priceRanges:['Any Price','0 – 1,500 MAD','1,500 – 4,000 MAD','4,000 – 10,000 MAD','10,000+ MAD'],
  },
  'boots': {
    label:'Boots',
    hero:'https://images.pexels.com/photos/1598508/pexels-photo-1598508.jpeg?auto=compress&w=1600',
    desc:'Luxury boots from ankle to knee — crafted for every occasion.',
    count:'876',
    brands:['Bottega Veneta','Gucci','Prada','Isabel Marant','Stuart Weitzman','UGG'],
    priceRanges:['Any Price','0 – 2,000 MAD','2,000 – 8,000 MAD','8,000 – 20,000 MAD','20,000+ MAD'],
  },
  'loafers': {
    label:'Loafers',
    hero:'https://images.pexels.com/photos/1124466/pexels-photo-1124466.jpeg?auto=compress&w=1600',
    desc:'Effortless luxury loafers for every setting.',
    count:'654',
    brands:['Gucci','Tod\'s','Church\'s','Prada','Hermès','Bottega Veneta'],
    priceRanges:['Any Price','0 – 2,000 MAD','2,000 – 8,000 MAD','8,000 – 20,000 MAD','20,000+ MAD'],
  },
  'sandals': {
    label:'Sandals',
    hero:'https://images.pexels.com/photos/3261069/pexels-photo-3261069.jpeg?auto=compress&w=1600',
    desc:'Designer sandals for every season and occasion.',
    count:'932',
    brands:['Hermès','Valentino','Jimmy Choo','Ancient Greek Sandals','Birkenstock','Havaianas'],
  ...
  'all-shoes': {
    ...
    desc:'Browse the complete SouKni footwear collection.',
    count:'3,412',
    brands:['Christian Louboutin','Gucci','Prada','Nike','Adidas','Hermès','Bottega Veneta'],
    priceRanges:['Any Price','0 – 2,000 MAD','2,000 – 8,000 MAD','8,000 – 20,000 MAD','20,000+ MAD'],
  },
}
...
const SHOE_IMGS = [
  ...
  'https://images.pexels.com/photos/1124466/pexels-photo-1124466.jpeg?auto=compress&w=400',
  'https://images.pexels.com/photos/336372/pexels-photo-336372.jpeg?auto=compress&w=400',
  'https://images.pexels.com/photos/3261069/pexels-photo-3261069.jpeg?auto=compress&w=400',
]
...
function ListingCard({ brand, title, price, location, size, condition, img, badge }: any) {
  ...
  return (
    <article onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{ padding:'16px 18px', flex:1, display:'flex', flexDirection:'column' as const }}>
        ...
        {size && <p style={{ fontSize:'10px', ...CB, color:C.muted, marginBottom:'2px' }}>Size EU {size}</p>}
        ...
        <div style={{ marginTop:'auto', display:'flex', gap:'8px' }}>
          ...
          >Message</button>
          <a href="https://wa.me/212600000000?text=Hi%2C%20I%20found%20your%20listing%20on%20SouKni!" target="_blank" rel="noopener noreferrer" style={{ flex:1, backgroundColor:'#25D366', color:'white', border:'none', padding:'9px', borderRadius:'12px', fontSize:'10px', ...CB, textTransform:'uppercase' as const, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'4px', textDecoration:'none' }}>
            💬 WhatsApp
          </a>
        </div>
      </div>
...
function makeListings(cat: string, count: number) {
  const brandMap: Record<string,string[]> = {
    'heels-pumps': ['Christian Louboutin','Manolo Blahnik','Jimmy Choo','YSL','Valentino'],
    'sneakers':    ['Nike','Jordan','Adidas','Golden Goose','Off-White','Balenciaga','New Balance'],
    'boots':       ['Bottega Veneta','Gucci','Prada','Isabel Marant','Stuart Weitzman'],
    'loafers':     ["Gucci","Tod's","Prada","Hermès","Church's","Bottega Veneta"],
    'sandals':     ['Hermès','Valentino','Jimmy Choo','Ancient Greek','Birkenstock'],
    'all-shoes':   ['Christian Louboutin','Gucci','Nike','Prada','Hermès','Bottega Veneta'],
  }
  const titleMap: Record<string,string[]> = {
    'heels-pumps': ['So Kate 120mm','Hangisi 105','Romy Pump','Tribute 105','Rockstud Pump'],
    'sneakers':    ['Air Jordan 1','Superstar GGDB','Ultra Boost','990v5','Air Force 1'],
    'boots':       ['Lug Chelsea','Horsebit Boot','Monolith Boot','Deva Boot','Stuart Boot'],
    'loafers':     ['Horsebit Loafer','Gommino Driver','Penny Loafer','Mocassin','Tassel Loafer'],
    'sandals':     ['Oran Sandal','Rockstud Flat','Romy Sandal','Greek Sandal','Arizona'],
    'all-shoes':   ['So Kate','Horsebit','Air Jordan 1','Monolith Boot','Oran Sandal'],
  }
  const brands = brandMap[cat] || brandMap['all-shoes']
  const titles = titleMap[cat] || titleMap['all-shoes']
  ...
  return Array.from({length:count},(_,i)=>({
    ...
    title:     `${titles[i%titles.length]} — ${['Black','White','Nude','Red','Tan','Navy'][i%6]}`,
    price:     1800 + ((i*1973)%28000),
    location:  locs[i%locs.length],
    size:      sizes[i%sizes.length],
    condition: conds[i%conds.length],
    img:       SHOE_IMGS[i%SHOE_IMGS.length],
...
export default function ShoesCategoryPage() {
  ...
  const catSlug  = (params?.category as string) || 'all-shoes'
  const catData  = CATEGORIES[catSlug] || CATEGORIES['all-shoes']
  ...
  function mapDbRowToCard(row: any) {
    return {
      ...
      img: (row.images && row.images[0]) || SHOE_IMGS[0],
    ...
    <div style={{ ...UB, backgroundColor:C.surface, color:C.ink, minHeight:'100vh' }}>
      ...
      <section style={{ position:'relative', height:'360px', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center' as const, maxWidth:'860px', padding:'0 24px', width:'100%' }}>
          <p style={{ fontSize:'11px', ...UB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.2em', marginBottom:'12px' }}>FASHION › SHOES</p>
        ...
        <div style={{ backgroundColor:'rgba(255,255,255,0.97)', backdropFilter:'blur(16px)', border:'1px solid rgba(107,122,118,0.12)', borderRadius:'100px', boxShadow:'0 12px 40px rgba(0,0,0,0.08)', display:'flex', alignItems:'stretch', height:'68px' }}>
          ...
          <div style={{ flex:1.8, padding:'0 22px', display:'flex', flexDirection:'column' as const, justifyContent:'center' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
              ...
              <input type="text" value={keyword} onChange={e=>setKeyword(e.target.value)} placeholder={`e.g. ${catData.brands[0]}, Size 38...`}
      ...
      <main style={{ maxWidth:'1280px', margin:'0 auto', padding:'32px 24px 80px' }}>
        ...
        <div style={{ backgroundColor:'white', borderRadius:'20px', padding:'20px 24px', marginBottom:'32px', border:'1px solid rgba(107,122,118,0.1)', boxShadow:'0 2px 8px rgba(0,0,0,0.04)' }}>
          ...
          <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' as const }}>
            <button style={{ padding:'7px 16px', borderRadius:'100px', fontSize:'10px', ...UB, border:`1px solid ${C.mint}`, backgroundColor:C.mint, color:C.ink, cursor:'pointer' }}>All Brands</button>
            {catData.brands.map(brand=>(
              <button key={brand}
                style={{ padding:'7px 16px', borderRadius:'100px', fontSize:'10px', ...UB, border:'1px solid rgba(107,122,118,0.2)', backgroundColor:'transparent', color:C.muted, cursor:'pointer', transition:'all 0.15s' }}
        ...
        <section style={{ marginBottom:'64px' }}>
          <h3 style={{ fontSize:'clamp(18px,2.5vw,24px)', ...UB, color:C.ink, textTransform:'uppercase' as const, marginBottom:'20px' }}>Explore Other Shoe Categories</h3>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:'16px' }}>
            {ALL_CATS.filter(c=>c.slug!==catSlug).map(cat=>(
              <Link key={cat.slug} href={`/${locale}/fashion/shoes/${cat.slug}`}
                style={{ backgroundColor:'white', borderRadius:'20px', padding:'24px 20px', textAlign:'center' as const, border:'1px solid rgba(107,122,118,0.1)', textDecoration:'none', transition:'all 0.2s', display:'block' }}
        ...
        <div style={{ textAlign:'center' as const }}>
          <Link href={`/${locale}/fashion/shoes`}
