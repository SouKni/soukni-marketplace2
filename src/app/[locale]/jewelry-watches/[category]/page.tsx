...
const CATEGORIES: Record<string,{ label:string; hero:string; desc:string; count:string; brands:string[]; priceRanges:string[]; metals?:string[] }> = {
  'all-items': {
    label:'All Jewelry & Watches',
    hero:'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=1600',
    desc:'Browse the complete SouKni jewelry and watches collection.',
    count:'4,218',
    brands:['Cartier','Van Cleef & Arpels','Rolex','Bulgari','Tiffany & Co','Patek Philippe','Audemars Piguet','Harry Winston'],
    priceRanges:['Any Price','0 – 5,000 MAD','5,000 – 20,000 MAD','20,000 – 80,000 MAD','80,000 – 200,000 MAD','200,000+ MAD'],
    metals:['All Metals','18K Yellow Gold','18K White Gold','Platinum','18K Rose Gold','Steel','Two-Tone'],
  },
  'luxury-watches': {
    label:'Luxury Watches',
    hero:'https://images.pexels.com/photos/437037/pexels-photo-437037.jpeg?auto=compress&w=1600',
    desc:'Iconic timepieces — from Rolex sports models to Patek Philippe grand complications.',
    count:'892',
    ...
    metals:['All Materials','Steel','18K Yellow Gold','18K White Gold','Two-Tone','Platinum','Titanium'],
  },
  'rings': {
    label:'Rings',
    hero:'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=1600',
    desc:'Engagement rings, wedding bands, and fashion rings from the world\'s finest jewellers.',
    count:'1,284',
    brands:['Cartier','Tiffany & Co','Van Cleef & Arpels','Bulgari','Harry Winston','Graff','Piaget','De Beers'],
    priceRanges:['Any Price','0 – 5,000 MAD','5,000 – 20,000 MAD','20,000 – 60,000 MAD','60,000 – 150,000 MAD','150,000+ MAD'],
    metals:['All Metals','18K Yellow Gold','18K White Gold','Platinum','18K Rose Gold'],
  },
  'necklaces': {
    label:'Necklaces',
    hero:'https://images.pexels.com/photos/248077/pexels-photo-248077.jpeg?auto=compress&w=1600',
    desc:'Timeless necklaces and pendants from the world\'s most iconic maisons.',
    count:'976',
    brands:['Van Cleef & Arpels','Cartier','Tiffany & Co','Bulgari','Mikimoto','Chopard','Harry Winston','Piaget'],
    priceRanges:['Any Price','0 – 5,000 MAD','5,000 – 25,000 MAD','25,000 – 80,000 MAD','80,000+ MAD'],
    metals:['All Metals','18K Yellow Gold','18K White Gold','Platinum'],
  },
  'bracelets': {
    label:'Bracelets',
    hero:'https://images.pexels.com/photos/1395306/pexels-photo-1395306.jpeg?auto=compress&w=1600',
    desc:'Luxury bracelets from Love bangles to diamond tennis bracelets.',
    count:'754',
    brands:['Cartier','Van Cleef & Arpels','Bulgari','Tiffany & Co','David Yurman','Chopard','Piaget'],
    priceRanges:['Any Price','0 – 5,000 MAD','5,000 – 25,000 MAD','25,000 – 80,000 MAD','80,000+ MAD'],
    metals:['All Metals','18K Yellow Gold','18K White Gold','18K Rose Gold','Platinum'],
  },
  'earrings': {
    label:'Earrings',
    hero:'https://images.pexels.com/photos/1413420/pexels-photo-1413420.jpeg?auto=compress&w=1600',
    desc:'Statement earrings from classic diamond studs to dramatic chandelier drops.',
    count:'312',
    brands:['Cartier','Chanel','Dior','Bulgari','Tiffany & Co','Van Cleef & Arpels','Piaget','Boucheron'],
  ...
  'vintage-antique': {
    ...
    hero:'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=1600',
    desc:'Estate jewelry and vintage timepieces from Art Deco to Mid-Century Modern.',
    count:'284',
    brands:['Cartier Vintage','Van Cleef Vintage','Rolex Vintage','Patek Vintage','Bulgari Archive','Omega Vintage'],
    priceRanges:['Any Price','0 – 5,000 MAD','5,000 – 20,000 MAD','20,000 – 60,000 MAD','60,000+ MAD'],
    metals:['All Eras','Art Deco','Retro 1940s','Mid-Century','1960s-70s','1980s-90s'],
  },
}

const ALL_CATS = [
  { label:'All Items',        slug:'all-items'       },
  { label:'Luxury Watches',   slug:'luxury-watches'  },
  { label:'Rings',            slug:'rings'           },
  { label:'Necklaces',        slug:'necklaces'       },
  { label:'Bracelets',        slug:'bracelets'       },
  { label:'Earrings',         slug:'earrings'        },
  { label:'Vintage & Antique',slug:'vintage-antique' },
]
...
function ListingCard({ id, brand, title, price, location, condition, img, badge, metal, phone, sellerId }: any) {
...
function makeListings(cat: string, count: number) {
  const titleMap: Record<string,string[]> = {
    'all-items':      ['Love Ring 18K','Alhambra Necklace','Datejust 41','Love Bracelet','Serpenti Ring','Diamond Solitaire'],
    'luxury-watches': ['Datejust 41 Oyster','Nautilus 5711/1A','Royal Oak 15400','RM 011 Felipe','Santos de Cartier','Speedmaster Pre-Moon','Perpetual Calendar','Submariner Date'],
    'rings':          ['Love Ring 18K YG','Solitaire 1ct Diamond','Engagement Ring Platinum','Band Ring Diamonds','Eternity Ring WG','Fashion Ring Ruby','Promise Ring','Stacking Ring Set'],
    'necklaces':      ['Alhambra Long 20M','Diamond Pendant 0.5ct','Pearl Necklace Mikimoto','Gold Chain 18K','Tennis Necklace','Choker Diamond','Sautoir Perles','Cross Pendant'],
    'bracelets':      ['Love Bracelet 18K WG','Alhambra Vintage 5M','Serpenti Bracelet','Tennis Bracelet 5ct','T Wire Bracelet','Knot Bracelet 18K','Cuff Bracelet Diamond'],
    'earrings':       ['Diamond Studs 1ct','Drop Earrings Gold','Hoop Earrings 18K','Chandelier Diamonds','Pearl Studs Mikimoto','Gold Hoops 20mm','Clip Earrings Vintage'],
    'vintage-antique':['Art Deco Diamond Ring','Retro Brooch 1940s','Cocktail Ring 1950s','Pearl Choker 1960s','Enamel Bracelet','Bakelite Bangle','Cameo Brooch','Mourning Ring'],
  }
  const metalMap: Record<string,string[]> = {
    'luxury-watches': ['Steel','18K Yellow Gold','18K White Gold','Two-Tone','Platinum'],
    ...
    'vintage-antique':['Art Deco c.1920','Retro c.1940','Mid-Century c.1955','Vintage c.1965'],
    'all-items':      ['18K Yellow Gold','18K White Gold','Steel','Platinum'],
  }
  const cat_data = CATEGORIES[cat] || CATEGORIES['all-items']
  const titles  = titleMap[cat] || titleMap['all-items']
  const metals  = metalMap[cat] || metalMap['all-items']
...
export default function JewelryCategoryPage() {
  ...
  const catSlug  = (params?.category as string) || 'all-items'
  const catData  = CATEGORIES[catSlug] || CATEGORIES['all-items']
  ...
  const [metal,        setMetal       ] = useState(catData.metals?.[0] || 'All Metals')
  ...
  function DDrop({ label, value, options, open, setOpen, onChange }: any) {
    return (
      <div style={{ position:'relative', flex:1 }}>
        <button onClick={()=>{ setOpen(!open); setCityOpen(false); setPriceOpen(false); setMetalOpen(false) }}
    ...
    <div style={{ ...UB, backgroundColor:C.surface, color:C.ink, minHeight:'100vh' }}>
      ...
      <section style={{ position:'relative', height:'360px', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center' as const, maxWidth:'860px', padding:'0 24px', width:'100%' }}>
          <p style={{ fontSize:'11px', ...UB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.2em', marginBottom:'12px' }}>THE VAULT › JEWELRY & WATCHES</p>
      ...
      {/* FILTER BAR with Metal/Era dropdown */}
        ...
        <div style={{ backgroundColor:'rgba(255,255,255,0.97)', backdropFilter:'blur(16px)', border:'1px solid rgba(107,122,118,0.12)', borderRadius:'100px', boxShadow:'0 12px 40px rgba(0,0,0,0.08)', display:'flex', alignItems:'stretch', height:'68px' }}>
          ...
          <div style={{ flex:1.8, padding:'0 22px', display:'flex', flexDirection:'column' as const, justifyContent:'center' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
              ...
              <input type="text" value={keyword} onChange={e=>setKeyword(e.target.value)} placeholder={`e.g. ${catData.brands[0]}, Diamond...`}
          ...
          <div style={{ width:'1px', backgroundColor:'rgba(107,122,118,0.12)', margin:'12px 0' }} />
          <DDrop label={catSlug==='vintage-antique'?'ERA':'METAL'} value={metal} options={catData.metals||['All Metals']} open={metalOpen} setOpen={setMetalOpen} onChange={setMetal} />
          <div style={{ width:'1px', backgroundColor:'rgba(107,122,118,0.12)', margin:'12px 0' }} />
      ...
      <main style={{ maxWidth:'1280px', margin:'0 auto', padding:'32px 24px 80px' }}>
        ...
        <Breadcrumb
          items={[
            ...
            { label:'Jewelry & Watches', href:`/${locale}/jewelry-watches` },
            { label:catData.label },
          ]}
          mutedColor={C.muted}
          inkColor={C.ink}
        ...
        <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' as const, marginBottom:'20px' }}>
          {ALL_CATS.map(cat=>(
            <Link key={cat.slug} href={`/${locale}/jewelry-watches/${cat.slug}`}
        ...
        <CategoryFooterNav
          ...
          related={ALL_CATS.filter(c=>c.slug!==catSlug).map(cat=>({ label:cat.label, href:`/${locale}/jewelry-watches/${cat.slug}` }))}
          backHref={`/${locale}/jewelry-watches`}
          backLabel="Back to Jewelry & Watches"
          inkColor={C.ink}
