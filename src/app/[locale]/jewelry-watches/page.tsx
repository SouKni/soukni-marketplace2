...
const ALL_SUBCATS = [
  { slug:'luxury-watches',  label:'Luxury Watches',    emoji:'⌚', image:'https://images.pexels.com/photos/437037/pexels-photo-437037.jpeg?auto=compress&w=600' },
  { slug:'rings',           label:'Rings',              emoji:'💍', image:'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=600' },
  { slug:'necklaces',       label:'Necklaces',          emoji:'📿', image:'https://images.pexels.com/photos/248077/pexels-photo-248077.jpeg?auto=compress&w=600' },
  { slug:'bracelets',       label:'Bracelets',          emoji:'⛓️', image:'https://images.pexels.com/photos/1395306/pexels-photo-1395306.jpeg?auto=compress&w=600' },
  { slug:'earrings',        label:'Earrings',           emoji:'💎', image:'https://images.pexels.com/photos/1413420/pexels-photo-1413420.jpeg?auto=compress&w=600' },
  { slug:'vintage-antique', label:'Vintage & Antique',  emoji:'🕰️', image:'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=600' },
]

const topChoices = [
  { id:'jw1', title:'Cartier Love Bracelet — 18K Yellow Gold, Full Set',   price:32000,  location:'Casablanca', rating:4.9, reviews:41, image:'https://images.pexels.com/photos/1395306/pexels-photo-1395306.jpeg?auto=compress&w=800', desc:'Authentic Cartier Love bracelet in 18K yellow gold, size 17. Comes with original box, papers and screwdriver. Barely worn, pristine condition — a timeless icon.' },
  { id:'jw2', title:'Rolex Datejust 41 — Steel & White Gold',              price:145000, location:'Rabat',      rating:4.9, reviews:37, image:'https://images.pexels.com/photos/437037/pexels-photo-437037.jpeg?auto=compress&w=800',  desc:'2022 Rolex Datejust 41 on Jubilee bracelet, fluted white gold bezel, silver dial. Full set with box, card and warranty. Serviced and ready to wear.' },
  { id:'jw3', title:'Van Cleef & Arpels Alhambra Necklace — 20-Motif',     price:58000,  location:'Marrakech',  rating:4.8, reviews:25, image:'https://images.pexels.com/photos/248077/pexels-photo-248077.jpeg?auto=compress&w=800',  desc:'Vintage Alhambra 20-motif necklace in 18K yellow gold with mother-of-pearl. Iconic design, excellent condition, authenticated by an independent gemologist.' },
]

const bentoListings = [
  { id:'bj1', title:'Diamond Tennis Bracelet — 5ct Total Weight',    price:68000, location:'Casablanca', image:'https://images.pexels.com/photos/1395306/pexels-photo-1395306.jpeg?auto=compress&w=600' },
  { id:'bj2', title:'Omega Speedmaster Moonwatch Professional',      price:42000, location:'Rabat',      image:'https://images.pexels.com/photos/437037/pexels-photo-437037.jpeg?auto=compress&w=600' },
  { id:'bj3', title:'18K Gold Hoop Earrings — Handcrafted',          price:6200,  location:'Fès',        image:'https://images.pexels.com/photos/1413420/pexels-photo-1413420.jpeg?auto=compress&w=600' },
  { id:'bj4', title:'Tiffany & Co Solitaire Engagement Ring — 1.2ct',price:89000, location:'Marrakech',  image:'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=600' },
  { id:'bj5', title:'Vintage Patek Philippe Calatrava — 1970s',      price:210000,location:'Casablanca', image:'https://images.pexels.com/photos/248077/pexels-photo-248077.jpeg?auto=compress&w=600' },
]

const discoveryGrid = [
  { id:'d1', title:'Bulgari Serpenti Viper Ring',              price:15500, location:'Rabat',      image:'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=600' },
  { id:'d2', title:'Chopard Happy Diamonds Pendant',           price:9800,  location:'Casablanca', image:'https://images.pexels.com/photos/248077/pexels-photo-248077.jpeg?auto=compress&w=600' },
  { id:'d3', title:'Men\'s Signet Ring — 18K Gold',            price:4200,  location:'Fès',        image:'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=600' },
  { id:'d4', title:'TAG Heuer Carrera Chronograph',            price:12500, location:'Marrakech',  image:'https://images.pexels.com/photos/437037/pexels-photo-437037.jpeg?auto=compress&w=600' },
  { id:'d5', title:'Pearl Strand Necklace — Akoya Cultured',   price:7600,  location:'Rabat',      image:'https://images.pexels.com/photos/248077/pexels-photo-248077.jpeg?auto=compress&w=600' },
  { id:'d6', title:'Cartier Trinity Ring — Three Gold',        price:5400,  location:'Casablanca', image:'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=600' },
  { id:'d7', title:'Vintage Longines Dress Watch — 1958',      price:8900,  location:'Tangier',    image:'https://images.pexels.com/photos/437037/pexels-photo-437037.jpeg?auto=compress&w=600' },
  { id:'d8', title:'Gold Bangle Set — 22K',                    price:11200, location:'Fès',        image:'https://images.pexels.com/photos/1395306/pexels-photo-1395306.jpeg?auto=compress&w=600' },
]
...
export default function JewelryWatchesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [activePill, setActivePill] = useState('All Jewelry')
  ...
  const visibleSubcats = ALL_SUBCATS
  ...
  function mapDbRowToItem(row: any) {
    return {
      ...
      image: (row.images && row.images[0]) || 'https://images.pexels.com/photos/1458867/pexels-photo-1458867.jpeg?auto=compress&w=800',
    }
  }
  ...
  const realDiscoveryGridAll = (dbListings.length >= 16 ? dbListings.slice(8, 16).map(mapDbRowToItem) : discoveryGrid)
    .filter((item: any) => applied.keyword.trim()==='' || item.title.toLowerCase().includes(applied.keyword.toLowerCase()))
  const PAGE_SIZE = Math.max(4, Math.ceil(realDiscoveryGridAll.length / 4))
  const totalPages = Math.max(1, Math.ceil(realDiscoveryGridAll.length / PAGE_SIZE))
  const clampedPage = Math.min(page, totalPages)
  const realDiscoveryGrid = realDiscoveryGridAll.slice((clampedPage - 1) * PAGE_SIZE, clampedPage * PAGE_SIZE)

  const pills = ['All Jewelry','Luxury Watches','Rings','Necklaces','Bracelets','Earrings','Vintage & Antique']
  ...
  return (
    <div style={{ fontFamily:'Inter, sans-serif', backgroundColor:'#f4fbf8', minHeight:'100vh' }}>
      ...
      <section style={{ position:'relative', height:'440px', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center' }}>
        <img src={HERO} alt="Jewelry & Watches" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }} />
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center', padding:'0 20px', maxWidth:'760px', width:'100%' }}>
          <h1 style={{ fontWeight:900, letterSpacing:'-0.05em', fontSize:'clamp(32px,5vw,56px)', color:'white', marginBottom:'12px', lineHeight:1.05 }}>
            Jewelry &amp;<br />Watches in Rabat.
          </h1>
          <p style={{ fontSize:'15px', color:'rgba(255,255,255,0.82)', marginBottom:'32px' }}>Fine jewelry, luxury watches &amp; vintage pieces from Morocco's most trusted sellers</p>
          <div style={{ display:'flex', alignItems:'stretch', backgroundColor:'rgba(255,255,255,0.12)', backdropFilter:'blur(24px)', border:'1px solid rgba(255,255,255,0.25)', borderRadius:'100px', overflow:'hidden', maxWidth:'680px', margin:'0 auto' }}>
            <div style={{ display:'flex', flexDirection:'column' as const, padding:'14px 20px', flex:'0 0 180px', borderRight:'1px solid rgba(255,255,255,0.2)', gap:'2px' }}>
              ...
              <input value={city} onChange={e=>setCity(e.target.value)} placeholder="Rabat" style={{ backgroundColor:'transparent', border:'none', outline:'none', fontSize:'14px', fontWeight:600, color:'white', padding:0, width:'100%' }} />
            </div>
            <div style={{ display:'flex', flexDirection:'column' as const, padding:'14px 20px', flex:1, borderRight:'1px solid rgba(255,255,255,0.2)', gap:'2px' }}>
              ...
              <input value={keyword} onChange={e=>setKeyword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&applySearch()} placeholder="Rolex, diamond ring, gold necklace..." style={{ backgroundColor:'transparent', border:'none', outline:'none', fontSize:'14px', fontWeight:600, color:'white', padding:0, width:'100%' }} />
            </div>
      ...
      <div style={{ maxWidth:'1440px', margin:'-28px auto 0', padding:'0 40px', position:'relative', zIndex:30 }}>
        <div style={{ backgroundColor:'rgba(255,255,255,0.9)', backdropFilter:'blur(20px)', borderRadius:'100px', padding:'10px 10px 10px 24px', boxShadow:'0 8px 40px rgba(0,0,0,0.12)', border:'1px solid rgba(255,255,255,0.6)', display:'flex', alignItems:'center' }}>
          {[['City','Rabat'],['Keyword','Rolex, diamond ring, gold necklace...'],['Category','All Categories'],['Price','Any Range'],['Filters','All Filters']].map(([l,v],i)=>(
      ...
      <div style={{ maxWidth:'1440px', margin:'32px auto 0', padding:'0 40px 64px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'4px' }}>
          <h2 style={{ fontWeight:900, letterSpacing:'-0.05em', fontSize:'22px', color:'#161d1b' }}>Jewelry &amp; Watches in Rabat</h2>
          <div style={{ display:'flex', gap:'8px' }}>
        ...
        <p style={{ fontSize:'14px', color:'#6b7a76', marginBottom:'24px' }}>4,218 Jewelry &amp; Watches across Morocco</p>
        ...
        <section style={{ marginBottom:'32px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:`repeat(${ALL_SUBCATS.length},1fr)`, gap:'10px', marginBottom:'12px' }}>
            {visibleSubcats.map(sub=>(
              <Link key={sub.slug} href={`/${locale}/jewelry-watches/${sub.slug}`} style={{ textDecoration:'none' }}>
        ...
        {/* BENTO COLLECTION */}
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontWeight:900, letterSpacing:'-0.05em', fontSize:'22px', color:'#22d4a8', marginBottom:'16px' }}>SouKni Jewelry Collection</h2>
        ...
        <section style={{ marginBottom:'40px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
            <h2 style={{ fontWeight:900, letterSpacing:'-0.05em', fontSize:'20px', color:'#161d1b' }}>More Jewelry &amp; Watches</h2>
        ...
        <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:'8px', marginBottom:'48px' }}>
          {Array.from({length:totalPages},(_,i)=>i+1).map(p=>(
            <button key={p} onClick={()=>setPage(p)} style={{ width:'36px', height:'36px', borderRadius:'10px', border:clampedPage===p?'none':'1px solid #e2e8f0', backgroundColor:clampedPage===p?'#22d4a8':'white', color:clampedPage===p?'white':'#161d1b', fontWeight:700, fontSize:'13px', cursor:'pointer' }}>{p}</button>
          ))}
          <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={clampedPage>=totalPages} style={{ padding:'0 16px', height:'36px', borderRadius:'10px', border:'1px solid #e2e8f0', backgroundColor:'white', color:'#161d1b', fontWeight:700, fontSize:'13px', cursor:clampedPage>=totalPages?'not-allowed':'pointer', opacity:clampedPage>=totalPages?0.4:1, display:'flex', alignItems:'center', gap:'4px' }}>Next <ChevronRight size={14} /></button>
        </div>
        ...
        <section style={{ borderRadius:'40px', background:'linear-gradient(135deg,#22d4a8,#0f9b8e)', padding:'56px 48px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'40px', flexWrap:'wrap' as const, marginBottom:'64px' }}>
          <div>
            ...
            <p style={{ fontSize:'15px', color:'rgba(255,255,255,0.85)', maxWidth:'480px', lineHeight:1.7 }}>List your jewelry, watches and fine accessories for free and reach Morocco's most discerning buyers.</p>
