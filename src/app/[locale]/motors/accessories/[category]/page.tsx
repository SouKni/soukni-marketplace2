...
const CATEGORIES: Record<string,{
  ...
  'all-parts': {
    ...
    desc:'Browse the complete SouKni parts and accessories marketplace.',
    count:'8,340',
    brands:['Bosch','Michelin','OEM','Pioneer','Brembo','Sachs','Denso','Continental'],
    priceRanges:['Any Range','0 – 500 MAD','500 – 1,500 MAD','1,500 – 3,500 MAD','3,500+ MAD'],
  },
  'engine-mechanical': {
    label:'Engine & Mechanical',
    hero:'https://images.pexels.com/photos/892522/pexels-photo-892522.jpeg?auto=compress&w=1600',
    desc:'Alternators, timing belts, filters, and mechanical components.',
    count:'2,410',
    brands:['Bosch','Sachs','Denso','OEM'],
    priceRanges:['Any Range','0 – 400 MAD','400 – 1,200 MAD','1,200 – 3,000 MAD','3,000+ MAD'],
  },
  'body-exterior': {
    label:'Body & Exterior',
    hero:'https://images.pexels.com/photos/244206/pexels-photo-244206.jpeg?auto=compress&w=1600',
    desc:'Bumpers, mirrors, panels, and exterior body parts.',
    count:'1,680',
    brands:['OEM','Continental'],
    priceRanges:['Any Range','0 – 800 MAD','800 – 2,000 MAD','2,000 – 5,000 MAD','5,000+ MAD'],
  },
  'interior': {
    label:'Interior',
    hero:'https://images.pexels.com/photos/1007410/pexels-photo-1007410.jpeg?auto=compress&w=1600',
    desc:'Seats, dashboards, trims, and interior accessories.',
    count:'940',
    brands:['OEM','Pioneer'],
    priceRanges:['Any Range','0 – 500 MAD','500 – 1,500 MAD','1,500+ MAD'],
  },
  'electronics-audio': {
    label:'Electronics & Audio',
    hero:'https://images.pexels.com/photos/1007410/pexels-photo-1007410.jpeg?auto=compress&w=1600',
    desc:'Head units, speakers, sensors, and car electronics.',
    count:'1,120',
    brands:['Pioneer','Bosch','Denso'],
    priceRanges:['Any Range','0 – 700 MAD','700 – 2,000 MAD','2,000 – 4,500 MAD','4,500+ MAD'],
  },
  'tires-wheels': {
    label:'Tires & Wheels',
    hero:'https://images.pexels.com/photos/2127039/pexels-photo-2127039.jpeg?auto=compress&w=1600',
    desc:'Tires, alloy wheels, and wheel accessories.',
    count:'1,050',
    brands:['Michelin','Continental','Brembo'],
    priceRanges:['Any Range','0 – 1,000 MAD','1,000 – 3,000 MAD','3,000 – 6,000 MAD','6,000+ MAD'],
  },
  'lighting': {
    label:'Lighting',
    hero:'https://images.pexels.com/photos/244206/pexels-photo-244206.jpeg?auto=compress&w=1600',
    desc:'Headlights, taillights, and lighting accessories.',
    count:'700',
    brands:['OEM','Bosch','Denso'],
    priceRanges:['Any Range','0 – 500 MAD','500 – 1,800 MAD','1,800+ MAD'],
  },
}
...
const PART_IMGS = [
  'https://images.pexels.com/photos/892522/pexels-photo-892522.jpeg?auto=compress&w=400',
  ...
  return (
    <article onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{ padding:'16px 18px', flex:1, display:'flex', flexDirection:'column' as const }}>
        ...
        {compatible && <p style={{ fontSize:'10px', ...CB, color:C.muted, marginBottom:'2px' }}>Fits: {compatible}</p>}
...
function makeListings(cat: string, count: number) {
  const titleMap: Record<string,string[]> = {
    'all-parts':          ['Alternator','Tire Set (4)','Headlight Assembly','Head Unit','Brake Disc Set','Side Mirror'],
    'engine-mechanical':  ['Alternator','Timing Belt Kit','Air Filter','Water Pump','Fuel Injector Set'],
    'body-exterior':      ['Front Bumper','Side Mirror Assembly','Fender Panel','Door Handle Set'],
    'interior':           ['Seat Cover Set','Dashboard Trim','Steering Wheel Cover','Floor Mats'],
    'electronics-audio':  ['Touchscreen Head Unit','Speaker Set','Reverse Camera','Parking Sensor Kit'],
    'tires-wheels':       ['Tire Set (4)','Alloy Wheel Set','Wheel Caps','Spare Tire'],
    'lighting':           ['Headlight Assembly','Taillight Set','Fog Light Kit','LED Bulb Set'],
  }
  const cat_data = CATEGORIES[cat] || CATEGORIES['all-parts']
  const titles = titleMap[cat] || titleMap['all-parts']
  const conds: CondT[] = ['new','used','refurbished','new','used','refurbished']
  const locs   = ['Casablanca','Rabat','Marrakech','Tangier','Agadir','Fès']
  const models = ['VW Golf/Passat','BMW 3 Series','Mercedes C-Class','Toyota Corolla','Renault Clio','Dacia Sandero']
  return Array.from({length:count},(_,i)=>({
    brand:      cat_data.brands[i%cat_data.brands.length],
    title:      titles[i%titles.length],
    price:      150 + ((i*731)%3400),
    location:   locs[i%locs.length],
    compatible: models[i%models.length],
    img:        PART_IMGS[i%PART_IMGS.length],
...
export default function AccessoriesCategoryPage() {
  ...
  const catSlug  = (params?.category as string) || 'all-parts'
  const catData  = CATEGORIES[catSlug] || CATEGORIES['all-parts']
    ...
    <div style={{ ...UB, backgroundColor:C.surface, color:C.ink, minHeight:'100vh' }}>
      ...
      <section style={{ position:'relative', height:'360px', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center' as const, maxWidth:'860px', padding:'0 24px', width:'100%' }}>
          <p style={{ fontSize:'11px', ...UB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.2em', marginBottom:'12px' }}>MOTORS › PARTS & ACCESSORIES</p>
          ...
          <div style={{ maxWidth:'620px', margin:'0 auto', backgroundColor:'rgba(255,255,255,0.12)', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,0.22)', borderRadius:'100px', padding:'6px', display:'flex', alignItems:'center', gap:'8px' }}>
            ...
            <button style={{ backgroundColor:C.mint, color:C.ink, border:'none', padding:'12px 28px', borderRadius:'100px', fontSize:'11px', ...UB, textTransform:'uppercase' as const, letterSpacing:'0.1em', cursor:'pointer' }}>FIND PARTS</button>
          </div>
        </div>
      </section>
      ...
      <main style={{ maxWidth:'1280px', margin:'0 auto', padding:'32px 24px 80px' }}>
        ...
        <Breadcrumb
          items={[
            ...
            { label:'Parts & Accessories', href:`/${locale}/motors/accessories` },
            { label:catData.label },
          ]}
          mutedColor={C.muted}
          inkColor={C.ink}
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'16px', marginBottom:'24px', flexWrap:'wrap' as const }}>
          <div>
            ...
            <p style={{ fontSize:'14px', color:C.mint, ...CB }}>{catData.count} Parts Listed</p>
          </div>
          <div style={{ display:'flex', gap:'10px', alignItems:'center' }}>
            <select value={sortBy} onChange={e=>setSortBy(e.target.value)}
              ...
              {['Most Recent','Price: Low to High','Price: High to Low'].map(s=><option key={s}>{s}</option>)}
            </select>
        ...
        <CategoryFooterNav
          relatedTitle="Explore Other Part Categories"
          related={ALL_CATS.filter(c=>c.slug!==catSlug).map(cat=>({ label:cat.label, href:`/${locale}/motors/accessories/${cat.slug}` }))}
          backHref={`/${locale}/motors/accessories`}
          backLabel="Back to All Parts"
          inkColor={C.ink}
          mintDkColor={C.mint}
        />
      </main>
