      ...
      <main style={{ maxWidth:'1280px', margin:'0 auto', padding:'32px 24px 80px' }}>
        ...
        <section style={{ marginBottom:'48px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'20px' }}>
            {listings.map((item,i)=><ListingCard key={(item as any).id||i} {...item} formatPrice={formatPrice} locale={locale} />)}
          </div>
        </section>
        ...
        <CategoryFooterNav
          relatedTitle="Explore Other Car Categories"
          related={ALL_CATS.filter(c=>c.slug!==catSlug).map(cat=>({ label:cat.label, href:`/${locale}/motors/cars/${cat.slug}` }))}
          backHref={`/${locale}/motors/cars`}
          backLabel="Back to All Cars"
          inkColor={C.ink}
          mintDkColor={C.mint}
        />
      </main>
