...
import { Heart, Search, ChevronDown, ChevronLeft, ChevronRight, SlidersHorizontal, MapPin, Plus, Diamond as DiamondIcon } from 'lucide-react'
...
const I = {
  ...
  c1:      'https://images.pexels.com/photos/3802510/pexels-photo-3802510.jpeg?auto=compress&w=600',
  c2:      'https://images.pexels.com/photos/116675/pexels-photo-116675.jpeg?auto=compress&w=600',
  c3:      'https://images.pexels.com/photos/1545743/pexels-photo-1545743.jpeg?auto=compress&w=600',
  c4:      'https://images.pexels.com/photos/210019/pexels-photo-210019.jpeg?auto=compress&w=600',
  bento1:  'https://images.pexels.com/photos/1638459/pexels-photo-1638459.jpeg?auto=compress&w=900',
  bento2:  'https://images.pexels.com/photos/120049/pexels-photo-120049.jpeg?auto=compress&w=600',
  bento3:  'https://images.pexels.com/photos/210019/pexels-photo-210019.jpeg?auto=compress&w=600',
  bento4:  'https://images.pexels.com/photos/116675/pexels-photo-116675.jpeg?auto=compress&w=600',
  inspect: 'https://images.pexels.com/photos/3802510/pexels-photo-3802510.jpeg?auto=compress&w=1200',
  autopro: 'https://images.pexels.com/photos/1545743/pexels-photo-1545743.jpeg?auto=compress&w=1200',
  g1:      'https://images.pexels.com/photos/3802510/pexels-photo-3802510.jpeg?auto=compress&w=400',
  g2:      'https://images.pexels.com/photos/116675/pexels-photo-116675.jpeg?auto=compress&w=400',
  g3:      'https://images.pexels.com/photos/1545743/pexels-photo-1545743.jpeg?auto=compress&w=400',
  g4:      'https://images.pexels.com/photos/210019/pexels-photo-210019.jpeg?auto=compress&w=400',
  g5:      'https://images.pexels.com/photos/1638459/pexels-photo-1638459.jpeg?auto=compress&w=400',
}
...
function FeaturedCard({ id, brand, title, price, location, img, badges, formatPrice, phone, sellerId }: any) {
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{ padding:'18px 20px' }}>
        ...
        <p style={{ fontSize:'20px', ...CB, color:C.mint, marginBottom:'4px' }}>{formatPrice(price)}</p>
...
function GridCard({ id, brand, title, price, img, badge, formatPrice, phone, sellerId }: any) {
  const { isFavorited, toggleFavorite } = useFavorites()
  const saved = isFavorited(id)
  const [hov,   setHov  ] = useState(false)
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{ padding:'14px 16px' }}>
        ...
        <p style={{ fontSize:'14px', ...CB, color:C.mint, marginBottom:'10px' }}>{formatPrice(price)}</p>
...
const featuredItems = [
  { brand:'BMW',           title:'M4 Competition — Carbon Pack',     price:785000,  location:'Maarif, Casablanca',  img:I.c1, badges:['featured','diamond']   },
  { brand:'Range Rover',    title:'Defender 110 — 2024',              price:1200000, location:'Agdal, Rabat',        img:I.c2, badges:['featured','diamond']   },
  { brand:'Audi',           title:'e-tron GT Quattro',                 price:1450000, location:'Gueliz, Marrakech',   img:I.c3, badges:['featured','certified'] },
  { brand:'Chevrolet',      title:'Corvette C8 — Low Mileage',         price:640000,  location:'Centre, Tanger',      img:I.c4, badges:['featured','new']       },
]

function makeGrid(count: number) {
  const brands = ['BMW','Mercedes-Benz','Audi','Porsche','Range Rover','Volkswagen','Toyota','Hyundai','Renault','Dacia','Ford','Peugeot']
  const titles = ['3 Series Sedan','GLE 63S Coupe','A6 Avant Quattro','Cayenne Turbo S','Velar Dynamic SE','Tiguan R-Line','Land Cruiser Prado','Tucson Hybrid','Talisman Initiale','Duster 4x4','Explorer ST','3008 GT Line']
  const imgs   = [I.g1,I.g2,I.g3,I.g4,I.g5]
  const badges: BadgeT[] = ['certified','diamond','featured','new','certified']
  return Array.from({length:count},(_,i)=>({
    brand: brands[i%brands.length],
    title: titles[i%titles.length],
    price: 180000 + ((i*27310)%1400000),
...
export default function CarsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [cityNeigh,    setCityNeigh   ] = useState('All Morocco')
  ...
  const kms         = ['Any Mileage','0 – 10,000 km','10,000 – 30,000 km','30,000 – 60,000 km','60,000 – 100,000 km','100,000+ km']
  const priceRanges = ['Any Price','0 – 150,000 MAD','150,000 – 400,000 MAD','400,000 – 800,000 MAD','800,000 – 1,500,000 MAD','1,500,000+ MAD']

  function DDrop({ label, value, options, open, setOpen, onChange }: any) {
    return (
      <div style={{ position:'relative', flex:1 }}>
        <button onClick={()=>{ setOpen(!open); setCityOpen(false); setYearOpen(false); setKmOpen(false); setPriceOpen(false) }}
  ...
  return (
    <div style={{ ...UB, backgroundColor:C.surface, color:C.ink, minHeight:'100vh' }}>
      ...
      <section style={{ position:'relative', height:'520px', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        <img src={I.hero} alt="Used Cars" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }} />
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center' as const, maxWidth:'960px', padding:'0 24px', width:'100%' }}>
          <h1 style={{ fontSize:'clamp(36px,6vw,64px)', ...UB, color:'white', marginBottom:'36px', lineHeight:1, textShadow:'0 4px 20px rgba(0,0,0,0.4)' }}>
            USED CARS FOR SALE.<br/><span style={{ color:C.mint }}>FIND YOUR PERFECT RIDE IN MOROCCO.</span>
          </h1>
          <div style={{ maxWidth:'780px', margin:'0 auto', backgroundColor:'rgba(255,255,255,0.12)', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,0.22)', borderRadius:'100px', padding:'8px', display:'flex', alignItems:'center' }}>
            ...
            <div style={{ flex:2, padding:'0 28px', display:'flex', flexDirection:'column' as const, gap:'2px' }}>
              <span style={{ fontSize:'9px', ...UB, color:'rgba(255,255,255,0.62)', textTransform:'uppercase' as const, letterSpacing:'0.15em' }}>MAKE & MODEL</span>
              <input type="text" value={makeModel} onChange={e=>setMakeModel(e.target.value)} placeholder="Search make, model, year..."
      ...
      <div style={{ maxWidth:'1280px', margin:'-40px auto 0', padding:'0 24px', position:'relative', zIndex:30 }}>
        <div style={{ backgroundColor:'rgba(255,255,255,0.97)', backdropFilter:'blur(16px)', border:'1px solid rgba(107,122,118,0.12)', borderRadius:'100px', boxShadow:'0 12px 40px rgba(0,0,0,0.08)', display:'flex', alignItems:'stretch', height:'72px' }}>
          <DDrop label="CITY & NEIGHBOURHOOD" value={cityNeigh} options={cityNeighs} open={cityOpen} setOpen={setCityOpen} onChange={setCityNeigh} />
          <div style={{ width:'1px', backgroundColor:'rgba(107,122,118,0.12)', margin:'12px 0' }} />
          <div style={{ flex:1.4, padding:'0 18px', display:'flex', flexDirection:'column' as const, justifyContent:'center' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
              ...
              <input type="text" value={makeModel} onChange={e=>setMakeModel(e.target.value)} placeholder="e.g. BMW, M4..."
          ...
          <div style={{ width:'1px', backgroundColor:'rgba(107,122,118,0.12)', margin:'12px 0' }} />
          <DDrop label="YEAR" value={year} options={years} open={yearOpen} setOpen={setYearOpen} onChange={setYear} />
          <div style={{ width:'1px', backgroundColor:'rgba(107,122,118,0.12)', margin:'12px 0' }} />
          <DDrop label="KM" value={km} options={kms} open={kmOpen} setOpen={setKmOpen} onChange={setKm} />
          <div style={{ width:'1px', backgroundColor:'rgba(107,122,118,0.12)', margin:'12px 0' }} />
          <DDrop label="PRICE (MAD)" value={price} options={priceRanges} open={priceOpen} setOpen={setPriceOpen} onChange={setPrice} />
          <div style={{ width:'1px', backgroundColor:'rgba(107,122,118,0.12)', margin:'12px 0' }} />
          <button style={{ display:'flex', alignItems:'center', gap:'8px', padding:'0 20px', background:'none', border:'none', cursor:'pointer', transition:'background 0.15s', flexShrink:0 }}
      ...
      <main style={{ maxWidth:'1280px', margin:'0 auto', padding:'32px 24px 80px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'16px', marginBottom:'24px', flexWrap:'wrap' as const }}>
          <div>
            ...
            <p style={{ fontSize:'14px', color:C.mint, ...CB }}>24,180 Ads across Morocco</p>
          </div>
        ...
        {/* ══ 7. FEATURED PREMIUM VEHICLES ═════════════════════ */}
        <section style={{ marginBottom:'48px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
            <div>
              <h3 style={{ fontSize:'clamp(18px,2.5vw,24px)', ...UB, color:C.ink, textTransform:'uppercase' as const, marginBottom:'4px' }}>Featured Premium Vehicles</h3>
              <p style={{ fontSize:'14px', color:C.muted }}>Hand-picked, inspected, and ready to drive</p>
            </div>
            <a href="#" style={{ fontSize:'12px', ...UB, color:C.mint, textDecoration:'none' }}>View All Featured</a>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'24px' }}>
            {featuredItems.map((item,i)=><FeaturedCard key={i} {...item} formatPrice={formatPrice} />)}
          </div>
        </section>

        {/* ══ 8. AUTO INSPECTION BANNER ════════════════════════ */}
        <section style={{ marginBottom:'64px' }}>
          <div style={{ backgroundColor:C.ink, borderRadius:'40px', padding:'56px 64px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'relative' as const, overflow:'hidden', minHeight:'240px', boxShadow:'0 16px 48px rgba(0,0,0,0.2)', flexWrap:'wrap' as const, gap:'24px' }}>
            ...
            <div style={{ position:'relative', zIndex:1, maxWidth:'480px' }}>
              ...
              <h2 style={{ fontSize:'clamp(24px,3.5vw,38px)', ...UB, color:'white', marginBottom:'20px', lineHeight:1.1 }}>Get your car professionally inspected before selling.</h2>
        ...
        {/* ══ 9. EXCLUSIVE PREMIUM VEHICLES BENTO ══════════════ */}
        <section style={{ marginBottom:'64px' }}>
          <h3 style={{ fontSize:'clamp(20px,3vw,32px)', ...UB, color:C.ink, textTransform:'uppercase' as const, marginBottom:'32px' }}>Exclusive Premium Vehicles</h3>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gridTemplateRows:'380px 380px', gap:'20px' }}>
            <div style={{ gridColumn:'1/3', gridRow:'1/3', backgroundColor:'white', borderRadius:'40px', overflow:'hidden', display:'flex', flexDirection:'column' as const, border:'1px solid rgba(107,122,118,0.1)' }}>
              <div style={{ flex:1, overflow:'hidden', position:'relative', minHeight:0 }}>
                <img src={I.bento1} alt="Porsche Cayenne" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
              ...
              <div style={{ padding:'24px 28px', flexShrink:0 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'6px', gap:'12px' }}>
                  <div>
                    <p style={{ fontSize:'11px', ...CB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.1em', marginBottom:'3px' }}>PORSCHE</p>
                    <h4 style={{ fontSize:'20px', ...CB, color:C.ink }}>Cayenne Turbo GT — Full Options</h4>
                  </div>
                  <span style={{ fontSize:'22px', ...CB, color:C.mint, flexShrink:0 }}>{formatPrice(1350000)}</span>
                </div>
                <p style={{ fontSize:'13px', color:C.muted, ...CB, marginBottom:'16px' }}>2023 · 8,200 km · Full service history · Casablanca, Maarif</p>
                <div style={{ display:'flex', gap:'10px' }}>
                  <MessageSellerButton listingId={undefined} sellerId={undefined} style={{ flex:1, border:`2px solid rgba(107,122,118,0.2)`, color:C.ink, backgroundColor:'transparent', padding:'12px', borderRadius:'16px', fontSize:'11px', ...CB, textTransform:'uppercase' as const, transition:'all 0.15s' }}
            ...
            <div style={{ gridColumn:'3', gridRow:'1', backgroundColor:'white', borderRadius:'32px', overflow:'hidden', display:'flex', flexDirection:'column' as const, border:'1px solid rgba(107,122,118,0.1)' }}>
              <div style={{ flex:1, overflow:'hidden', position:'relative', minHeight:0 }}>
                <img src={I.bento2} alt="Mercedes GLE" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
              ...
              <div style={{ padding:'16px 18px', flexShrink:0 }}>
                <p style={{ fontSize:'9px', ...CB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.1em', marginBottom:'2px' }}>MERCEDES-BENZ</p>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'6px', marginBottom:'10px' }}>
                  <h4 style={{ fontSize:'13px', ...CB, color:C.ink }}>GLE 63S AMG Coupe</h4>
                  <span style={{ fontSize:'14px', ...CB, color:C.mint, flexShrink:0 }}>{formatPrice(1100000)}</span>
                </div>
                <div style={{ display:'flex', gap:'6px' }}>
                  <MessageSellerButton listingId={undefined} sellerId={undefined} style={{ flex:1, border:`1px solid rgba(107,122,118,0.2)`, color:C.ink, backgroundColor:'transparent', padding:'8px', borderRadius:'12px', fontSize:'9px', ...CB }}>Message</MessageSellerButton>
            ...
            <div style={{ gridColumn:'4', gridRow:'1', backgroundColor:'white', borderRadius:'32px', overflow:'hidden', display:'flex', flexDirection:'column' as const, border:'1px solid rgba(107,122,118,0.1)' }}>
              <div style={{ flex:1, overflow:'hidden', position:'relative', minHeight:0 }}>
                <img src={I.bento3} alt="Corvette" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                <div style={{ position:'absolute', top:'12px', left:'12px' }}>
              ...
              <div style={{ padding:'16px 18px', flexShrink:0 }}>
                <p style={{ fontSize:'9px', ...CB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.1em', marginBottom:'2px' }}>CHEVROLET</p>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'6px', marginBottom:'10px' }}>
                  <h4 style={{ fontSize:'13px', ...CB, color:C.ink }}>Corvette C8 Stingray</h4>
                  <span style={{ fontSize:'14px', ...CB, color:C.mint, flexShrink:0 }}>{formatPrice(640000)}</span>
                </div>
                <div style={{ display:'flex', gap:'6px' }}>
                  <MessageSellerButton listingId={undefined} sellerId={undefined} style={{ flex:1, border:`1px solid rgba(107,122,118,0.2)`, color:C.ink, backgroundColor:'transparent', padding:'8px', borderRadius:'12px', fontSize:'9px', ...CB }}>Message</MessageSellerButton>
                  <WhatsAppButton phone={undefined} title="this listing" style={{ flex:1, padding:'8px', borderRadius:'12px', fontSize:'9px', ...CB }}>WhatsApp</WhatsAppButton>
            ...
            <div style={{ gridColumn:'3/5', gridRow:'2', backgroundColor:'white', borderRadius:'32px', overflow:'hidden', display:'flex', flexDirection:'row' as const, border:'1px solid rgba(107,122,118,0.1)' }}>
              <div style={{ width:'50%', overflow:'hidden', position:'relative' }}>
                <img src={I.bento4} alt="Range Rover" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                <div style={{ position:'absolute', top:'12px', left:'12px' }}>
              ...
              <div style={{ flex:1, padding:'28px 32px', display:'flex', flexDirection:'column' as const, justifyContent:'center' }}>
                <p style={{ fontSize:'11px', ...CB, color:C.mint, textTransform:'uppercase' as const, letterSpacing:'0.1em', marginBottom:'6px' }}>RANGE ROVER</p>
                <h4 style={{ fontSize:'18px', ...CB, color:C.ink, marginBottom:'6px' }}>Autobiography Long Wheelbase</h4>
                <p style={{ fontSize:'13px', color:C.muted, ...CB, marginBottom:'12px' }}>2023 · 18,000 km · Full BMW dealership history · Casablanca</p>
                <span style={{ fontSize:'22px', ...CB, color:C.mint, marginBottom:'20px', display:'block' }}>{formatPrice(1850000)}</span>
                <div style={{ display:'flex', gap:'10px' }}>
                  <MessageSellerButton listingId={undefined} sellerId={undefined} style={{ flex:1, border:`2px solid rgba(107,122,118,0.2)`, color:C.ink, backgroundColor:'transparent', padding:'12px', borderRadius:'16px', fontSize:'10px', ...CB, transition:'border-color 0.15s' }}
        ...
        <section style={{ marginBottom:'64px' }}>
          <div style={{ position:'relative', borderRadius:'40px', overflow:'hidden', height:'320px', display:'flex', alignItems:'center', boxShadow:'0 16px 48px rgba(0,0,0,0.15)', cursor:'pointer' }}>
            <img src={I.autopro} alt="Sell Faster" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }} />
            ...
            <div style={{ position:'relative', zIndex:1, padding:'0 64px', maxWidth:'560px' }}>
              ...
              <h2 style={{ fontSize:'clamp(26px,3.5vw,42px)', ...UB, color:'white', marginBottom:'18px', lineHeight:1.05 }}>Sell Your Car in Days, Not Months.</h2>
              <p style={{ fontSize:'17px', color:'rgba(255,255,255,0.9)', marginBottom:'28px', lineHeight:1.55 }}>Professional listing photos, priority placement, and verified buyer leads.</p>
              <button style={{ backgroundColor:C.mint, color:C.ink, border:'none', padding:'16px 40px', borderRadius:'100px', fontSize:'11px', ...UB, textTransform:'uppercase' as const, letterSpacing:'0.12em', cursor:'pointer', transition:'transform 0.2s' }}
                onMouseEnter={e=>e.currentTarget.style.transform='scale(1.04)'}
                onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}
        ...
        {/* ══ 11. CAR DISCOVERIES GRID ═════════════════════════ */}
        <section style={{ marginBottom:'48px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'28px' }}>
            <h3 style={{ fontSize:'clamp(18px,2.5vw,24px)', ...UB, color:C.ink, textTransform:'uppercase' as const }}>Car Discoveries</h3>
          ...
          {[gridItems.slice(0,4),gridItems.slice(4,8),gridItems.slice(8,12),gridItems.slice(12,16)].map((row,ri)=>(
            <div key={ri} style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'20px', marginBottom:'20px' }}>
              {row.map((item,j)=><GridCard key={j} {...item} formatPrice={formatPrice} />)}
            </div>
          ))}
        </section>
        ...
        <section style={{ marginBottom:'64px' }}>
          <div style={{ backgroundColor:C.mint, borderRadius:'40px', padding:'56px 64px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:`0 16px 48px ${C.mint}30`, flexWrap:'wrap' as const, gap:'28px' }}>
            <div style={{ maxWidth:'520px' }}>
              ...
              <p style={{ fontSize:'17px', color:`${C.ink}b3`, lineHeight:1.6 }}>Priority placement, vehicle authentication, and a dedicated buyer concierge. Sell your car 5x faster.</p>
            </div>
        ...
        <section style={{ marginBottom:'64px' }}>
          <div style={{ backgroundColor:C.cream, borderRadius:'40px', height:'440px', position:'relative' as const, overflow:'hidden', display:'flex', alignItems:'center', padding:'0 64px', border:'1px solid rgba(107,122,118,0.1)' }}>
            <div style={{ position:'relative', zIndex:1, maxWidth:'480px' }}>
              ...
              <p style={{ fontSize:'17px', color:C.muted, marginBottom:'36px', maxWidth:'400px', lineHeight:1.6 }}>Get early access to exclusive vehicle listings, inspections, and private dealer sales.</p>
              <div style={{ display:'flex', gap:'14px', flexWrap:'wrap' as const }}>
