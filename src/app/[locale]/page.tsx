...
const categories = [
  { label: 'Motors', count: '24,180', icon: '🚗', slug: 'motors', image: 'https://images.pexels.com/photos/3802510/pexels-photo-3802510.jpeg?auto=compress&w=600', color: '#006b5f' },
  { label: 'Property', count: '12,450', icon: '🏡', slug: 'property', image: 'https://images.pexels.com/photos/2581922/pexels-photo-2581922.jpeg?auto=compress&w=600', color: '#006b5f' },
  { label: 'The Vault', count: '840', icon: '💎', slug: 'vault', image: 'https://images.pexels.com/photos/190819/pexels-photo-190819.jpeg?auto=compress&w=600', color: '#8d4f00' },
  { label: 'Electronics', count: '38,400', icon: '📱', slug: 'electronics', image: 'https://images.pexels.com/photos/303383/pexels-photo-303383.jpeg?auto=compress&w=600', color: '#006b5f' },
  { label: 'Fashion', count: '6,388', icon: '👗', slug: 'fashion', image: 'https://images.pexels.com/photos/1152077/pexels-photo-1152077.jpeg?auto=compress&w=600', color: '#006b5f' },
  { label: 'Services', count: '9,200', icon: '🔧', slug: 'community', image: 'https://images.pexels.com/photos/4246120/pexels-photo-4246120.jpeg?auto=compress&w=600', color: '#006b5f' },
]
    ...
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div style={{ position: 'relative', height: isVault ? '240px' : '200px', overflow: 'hidden' }}>
        ...
        {item.badge && (
          <div style={{ position: 'absolute', top: '14px', left: '14px' }}>
            <span style={{ background: item.badge === 'Diamond' ? 'linear-gradient(135deg, #006b5f, #2dd4bf)' : 'rgba(255,255,255,0.92)', backdropFilter: 'blur(8px)', color: item.badge === 'Diamond' ? 'white' : '#006b5f', fontSize: '9px', fontWeight: 800, padding: '4px 12px', borderRadius: '100px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <p style={{ fontSize: '18px', fontWeight: 800, color: isVault ? '#8d4f00' : '#006b5f', letterSpacing: '-0.01em' }}>{item.price}</p>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button style={{ flex: 1, border: isVault ? '1.5px solid #8d4f00' : '1.5px solid #006b5f', color: isVault ? '#8d4f00' : '#006b5f', backgroundColor: 'transparent', padding: '10px', borderRadius: '100px', fontWeight: 700, fontSize: '13px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', transition: 'background 0.15s' }}
    ...
    <section style={{ backgroundColor: bg || 'transparent', padding: '72px 0' }}>
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 40px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '40px' }}>
          ...
          <Link href={`/${locale}${href}`} style={{ display: 'flex', alignItems: 'center', gap: '6px', color: isVault ? '#8d4f00' : '#006b5f', fontWeight: 700, fontSize: '14px', textDecoration: 'none', transition: 'gap 0.2s' }}
...
export default function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif', backgroundColor: '#f4fbf8', minHeight: '100vh' }}>
      ...
      {/* ── HERO ── */}
      <section style={{ position: 'relative', height: '620px', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', padding: '0 20px', maxWidth: '860px', width: '100%' }}>
          ...
          <div style={{ maxWidth: '720px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', gap: '0', backgroundColor: 'rgba(255,255,255,0.12)', backdropFilter: 'blur(24px)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '20px', padding: '8px', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 20px', borderRight: '1px solid rgba(255,255,255,0.15)', cursor: 'pointer' }}>
                <MapPin size={16} color="rgba(255,255,255,0.6)" />
                <span style={{ fontSize: '15px', color: 'rgba(255,255,255,0.8)', whiteSpace: 'nowrap' }}>All Morocco</span>
              </div>
              <button style={{ backgroundColor: '#2dd4bf', color: '#00201c', border: 'none', padding: '14px 32px', borderRadius: '14px', fontWeight: 800, fontSize: '14px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', letterSpacing: '0.05em', transition: 'all 0.15s', whiteSpace: 'nowrap' }}
      ...
      {/* ── STATS BAR ── */}
      <div style={{ backgroundColor: '#006b5f', padding: '0' }}>
      ...
      <section style={{ maxWidth: '1440px', margin: '0 auto', padding: '80px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gridTemplateRows: '240px 240px', gap: '16px' }}>
          {categories.map((cat, i) => (
            <Link key={cat.slug} href={`/${locale}/${cat.slug}`} style={{ textDecoration: 'none', gridRow: i === 0 ? 'span 2' : 'span 1' }}>
              <div style={{ position: 'relative', height: '100%', borderRadius: '24px', overflow: 'hidden', cursor: 'pointer' }}
                ...
                <div style={{ position: 'absolute', inset: 0, background: i === 0 ? 'linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.2) 60%, rgba(0,0,0,0) 100%)' : 'linear-gradient(to top, rgba(0,0,0,0.8), rgba(0,0,0,0.1))' }} />
                <div style={{ position: 'absolute', bottom: '0', left: '0', right: '0', padding: i === 0 ? '32px' : '20px' }}>
                  <p style={{ color: '#62fae3', fontSize: i === 0 ? '13px' : '11px', fontWeight: 700, marginBottom: '6px', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{cat.count} listings</p>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <h3 style={{ color: 'white', fontSize: i === 0 ? '32px' : '20px', fontWeight: 800, letterSpacing: '-0.02em' }}>{cat.label}</h3>
                    <div style={{ width: i === 0 ? '40px' : '32px', height: i === 0 ? '40px' : '32px', borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.2)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <ChevronRight size={i === 0 ? 20 : 16} color="white" />
                    </div>
                  </div>
                </div>
                {/* Icon badge */}
                <div style={{ position: 'absolute', top: '16px', left: '16px', fontSize: i === 0 ? '28px' : '20px' }}>{cat.icon}</div>
              </div>
            </Link>
          ))}
        </div>
      ...
      {/* ── LISTING SECTIONS ── */}
      <SectionRow title="Featured Motors" subtitle="The finest performance and luxury vehicles in Morocco" items={motors} bg="#f8fafc" href="/motors" locale={locale} />
        ...
        <div style={{ position: 'relative', borderRadius: '32px', overflow: 'hidden', height: '280px' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', padding: '48px' }}>
            <div style={{ maxWidth: '480px' }}>
              ...
              <Link href={`/${locale}/property`} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', backgroundColor: 'white', color: '#006b5f', padding: '14px 28px', borderRadius: '100px', fontWeight: 800, fontSize: '14px', textDecoration: 'none', transition: 'all 0.15s' }}
      ...
      {/* ── DIAMOND BANNER ── */}
      <section style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 40px 72px' }}>
        <div style={{ position: 'relative', borderRadius: '32px', overflow: 'hidden', background: 'linear-gradient(135deg, #006b5f 0%, #0f766e 50%, #2dd4bf 100%)', padding: '64px 56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '40px' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1, maxWidth: '560px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button style={{ backgroundColor: 'white', color: '#006b5f', border: 'none', padding: '14px 32px', borderRadius: '100px', fontWeight: 800, fontSize: '15px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', transition: 'all 0.15s' }}
                onMouseEnter={e => e.currentTarget.style.backgroundColor = '#62fae3'}
                onMouseLeave={e => e.currentTarget.style.backgroundColor = 'white'}>
                Get Verified Now
              </button>
          ...
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'center' }}>
            <div style={{ position: 'relative', width: '280px', height: '560px' }}>
              ...
              <div style={{ position: 'relative', width: '280px', height: '560px', backgroundColor: '#0f172a', borderRadius: '48px', border: '8px solid rgba(255,255,255,0.15)', overflow: 'hidden', boxShadow: '0 40px 80px rgba(0,0,0,0.3)' }}>
                ...
                <div style={{ padding: '40px 20px 20px', backgroundColor: '#006b5f', height: '160px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
