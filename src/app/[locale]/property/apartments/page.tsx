...
const APARTMENT_LISTINGS = [
  ...
  { id:'ap4', badge:'New Listing', title:'Duplex with Rooftop Terrace', price:'3,650,000', location:'Hay Riad, Rabat', beds:3, baths:2, area:165, image:'https://images.pexels.com/photos/1643384/pexels-photo-1643384.jpeg?auto=compress&w=700' },
  { id:'ap5', badge:'Exclusive', title:'Sea View Penthouse Apartment', price:'6,800,000', location:'Corniche, Casablanca', beds:4, baths:3, area:220, image:'https://images.pexels.com/photos/440731/pexels-photo-440731.jpeg?auto=compress&w=700' },
  { id:'ap6', badge:'Verified', title:'Bright 2-Bedroom Near Agdal', price:'1,850,000', location:'Agdal, Rabat', beds:2, baths:1, area:95, image:'https://images.pexels.com/photos/1029599/pexels-photo-1029599.jpeg?auto=compress&w=700' },
  { id:'ap7', badge:'New', title:'Family Apartment with Garden Access', price:'2,950,000', location:'Souissi, Rabat', beds:3, baths:2, area:140, image:'https://images.pexels.com/photos/2581922/pexels-photo-2581922.jpeg?auto=compress&w=700' },
  { id:'ap8', badge:'Verified', title:'Compact 1-Bed Investment Unit', price:'980,000', location:'Marrakech Gueliz', beds:1, baths:1, area:62, image:'https://images.pexels.com/photos/1396122/pexels-photo-1396122.jpeg?auto=compress&w=700' },
]
...
const newProjects = [
  ...
  { name:'Agdal Residences',   city:'Agdal, Rabat',            price:'From 950K MAD', image:'https://images.pexels.com/photos/2581922/pexels-photo-2581922.jpeg?auto=compress&w=200' },
]
      ...
      <div style={{ flex:1, padding:20, display:'flex', flexDirection:'column' as const, justifyContent:'space-between' }}>
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:6 }}>
            ...
            <span style={{ fontSize:11, color:C.muted, fontWeight:500 }}>Apartment</span>
          </div>
...
export default function ApartmentsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [propertyType,  setPropertyType]= useState('All Apartments')
  ...
  return (
    <div style={{ ...UB, backgroundColor:C.surface, color:C.ink, minHeight:'100vh' }}>
      ...
      <section style={{ position:'relative', height:520, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        <img src="https://images.pexels.com/photos/1396122/pexels-photo-1396122.jpeg?auto=compress&w=1600" alt="Apartments" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }} />
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center' as const, maxWidth:960, padding:'0 24px', width:'100%' }}>
          <h1 style={{ fontSize:'clamp(36px,6vw,64px)', ...UB, color:'white', marginBottom:36, lineHeight:1, textShadow:'0 4px 20px rgba(0,0,0,0.4)' }}>
            APARTMENTS.<br/><span style={{ color:C.mint }}>FIND YOUR ADDRESS.</span>
          </h1>
          <div style={{ maxWidth:780, margin:'0 auto', backgroundColor:'rgba(255,255,255,0.12)', backdropFilter:'blur(12px)', border:'1px solid rgba(255,255,255,0.22)', borderRadius:100, padding:8, display:'flex', alignItems:'center' }}>
            <div style={{ flex:1, padding:'0 28px', borderRight:'1px solid rgba(255,255,255,0.22)', display:'flex', flexDirection:'column' as const, gap:2 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:6, color:'white', fontSize:14, ...UB }}>Casablanca <ChevronDown size={14} /></div>
            </div>
            <div style={{ flex:2, padding:'0 28px', display:'flex', flexDirection:'column' as const, gap:2 }}>
              ...
              <input type="text" value={keyword} onChange={e=>setKeyword(e.target.value)} placeholder="Search neighborhood, building, project..."
      ...
      <div style={{ maxWidth:1280, margin:'-40px auto 0', padding:'0 24px', position:'relative', zIndex:30 }}>
        <div style={{ backgroundColor:'rgba(255,255,255,0.97)', backdropFilter:'blur(16px)', border:'1px solid rgba(107,122,118,0.12)', borderRadius:100, boxShadow:'0 12px 40px rgba(0,0,0,0.08)', display:'flex', alignItems:'stretch', height:72 }}>
          <DDrop label="LOCATION" value={location} options={['Cities & Neighbourhoods','Casablanca','Rabat','Marrakech','Fès','Tanger','Agadir','Meknès']} open={locOpen} setOpen={setLocOpen} onChange={setLocation} />
      ...
      <main style={{ maxWidth:1280, margin:'0 auto', padding:'32px 24px 80px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:16, marginBottom:24, flexWrap:'wrap' as const }}>
          <div>
            ...
            <p style={{ fontSize:14, color:C.mint, ...CB }}>8,420 Ads</p>
          </div>
          ...
          {ALL_CATS.map(cat=>(
            <Link key={cat.slug} href={`/${locale}/property/${cat.slug}`}
              style={{ padding:'10px 22px', borderRadius:100, fontSize:11, ...UB, textTransform:'uppercase' as const, letterSpacing:'0.1em', cursor:'pointer', transition:'all 0.2s', border:'1px solid', textDecoration:'none', display:'inline-block',
                backgroundColor: cat.slug==='apartments' ? C.mint : 'white', color: cat.slug==='apartments' ? 'white' : C.muted, borderColor: cat.slug==='apartments' ? C.mint : 'rgba(186,202,197,0.4)' }}>
              {cat.label}
            </Link>
          ))}
        ...
        {/* ══ MAIN GRID + SIDEBAR (from for-sale) ═══════════════ */}
          ...
          <div style={{ display:'flex', flexDirection:'column' as const, gap:20 }}>
            ...
            <div style={{ background:'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', borderRadius:16, padding:'28px 32px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'relative', overflow:'hidden' }}>
              ...
              <div style={{ position:'relative', zIndex:1 }}>
                <h2 style={{ fontSize:20, fontWeight:800, color:'white', marginBottom:6 }}>What's your apartment worth today?</h2>
            ...
            <div style={{ borderRadius:16, overflow:'hidden', border:'1px solid #f1f5f9' }}>
              <div style={{ background:'linear-gradient(135deg, #0f9b8e 0%, #22d4a8 100%)', padding:'28px 32px', display:'flex', alignItems:'center', gap:20 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize:13, color:'rgba(255,255,255,0.85)' }}>Explore apartments with immersive 360° tours and AI-powered videos.</p>
                </div>
              </div>
            </div>
            ...
            <div style={{ position:'relative', borderRadius:16, overflow:'hidden', height:220, cursor:'pointer' }}>
              ...
              <div style={{ position:'absolute', inset:0, background:'linear-gradient(to right, rgba(15,23,42,0.92) 0%, rgba(15,23,42,0.62) 60%, transparent 100%)', display:'flex', flexDirection:'column' as const, justifyContent:'center', padding:32 }}>
                <div style={{ maxWidth:380 }}>
                  ...
                  <p style={{ fontSize:13, color:'#cbd5e1', marginBottom:18, lineHeight:1.6 }}>Boost your apartment listings to over 2M monthly seekers.</p>
          ...
          <aside style={{ display:'flex', flexDirection:'column' as const, gap:20, position:'sticky', top:114 }}>
            <div style={{ backgroundColor:'white', borderRadius:16, padding:20, border:'1px solid #f1f5f9', boxShadow:'0 4px 20px rgba(0,0,0,0.05)' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
                <h3 style={{ fontWeight:800, color:C.ink, fontSize:14 }}>New Apartment Projects</h3>
            ...
            <div style={{ background:'rgba(34,212,168,0.06)', border:'2px solid rgba(34,212,168,0.2)', borderRadius:16, padding:20 }}>
              ...
              <p style={{ fontSize:12, color:'#475569', marginBottom:14, lineHeight:1.6 }}>Reach over 2M apartment seekers monthly in Morocco.</p>
            ...
            <div style={{ backgroundColor:'white', borderRadius:16, padding:20, border:'1px solid #f1f5f9', boxShadow:'0 4px 20px rgba(0,0,0,0.05)' }}>
              ...
              <p style={{ fontSize:12, color:C.muted, marginBottom:14, lineHeight:1.6 }}>We'll notify you when new apartments matching your search are listed.</p>
