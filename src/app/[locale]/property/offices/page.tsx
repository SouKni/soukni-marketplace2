...
const OFFICE_LISTINGS = [
  { id:'of1', badge:'Verified', badge2:'Prime', title:'Premium Retail Space in High-End Mall', price:'4,500,000', location:'Rabat, Souissi', area:120, image:'https://images.pexels.com/photos/2062426/pexels-photo-2062426.jpeg?auto=compress&w=700' },
  { id:'of2', badge:'New', title:'Modern Office Floor — CFC Tower', price:'6,200,000', location:'Casablanca Finance City', area:280, image:'https://images.pexels.com/photos/1029599/pexels-photo-1029599.jpeg?auto=compress&w=700' },
  { id:'of3', badge:'Exclusive', title:'Coworking Floor with Full Fit-Out', price:'3,850,000', location:'Marrakech Gueliz', area:210, image:'https://images.pexels.com/photos/1643384/pexels-photo-1643384.jpeg?auto=compress&w=700' },
  { id:'of4', badge:'Verified', title:'Warehouse & Logistics Facility', price:'8,900,000', location:'Casablanca, Ain Sebaa', area:1500, image:'https://images.pexels.com/photos/2581922/pexels-photo-2581922.jpeg?auto=compress&w=700' },
  { id:'of5', badge:'New Listing', title:'Showroom on Main Commercial Avenue', price:'5,400,000', location:'Rabat, Agdal', area:340, image:'https://images.pexels.com/photos/440731/pexels-photo-440731.jpeg?auto=compress&w=700' },
  { id:'of6', badge:'Verified', title:'Ground Floor Retail Unit', price:'2,900,000', location:'Tanger City Center', area:95, image:'https://images.pexels.com/photos/1396122/pexels-photo-1396122.jpeg?auto=compress&w=700' },
  { id:'of7', badge:'Exclusive', title:'Corporate HQ Building — Full Floor', price:'12,500,000', location:'Casablanca, Twin Center', area:650, image:'https://images.pexels.com/photos/2062426/pexels-photo-2062426.jpeg?auto=compress&w=700' },
  { id:'of8', badge:'Verified', title:'Boutique Retail Space Near Medina', price:'2,100,000', location:'Fès Ville Nouvelle', area:75, image:'https://images.pexels.com/photos/1029599/pexels-photo-1029599.jpeg?auto=compress&w=700' },
]


const newProjects = [
  { name:'CFC Business Tower',  city:'Casablanca Finance City', price:'From 5M MAD', image:'https://images.pexels.com/photos/2062426/pexels-photo-2062426.jpeg?auto=compress&w=200' },
  { name:'Rabat Commerce Hub',  city:'Agdal, Rabat',             price:'From 2.8M MAD', image:'https://images.pexels.com/photos/1029599/pexels-photo-1029599.jpeg?auto=compress&w=200' },
]
...
function PropertyCard({ prop }: { prop: typeof OFFICE_LISTINGS[0] }) {
      ...
      <div style={{ flex:1, padding:20, display:'flex', flexDirection:'column' as const, justifyContent:'space-between' }}>
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:6 }}>
            ...
            <span style={{ fontSize:11, color:C.muted, fontWeight:500 }}>Commercial</span>
          </div>
...
export default function OfficesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [propertyType, setPropertyType] = useState('All Commercial')
  const [price, setPrice]               = useState('Any Price')
  const [beds, setBeds]                 = useState('Any Size')
  ...
  const propertyTypes = ['All Commercial','Office Space','Retail Unit','Warehouse','Coworking Floor','Showroom']
  const priceRanges   = ['Any Price','0 – 1M MAD','1M – 3M MAD','3M – 8M MAD','8M+ MAD']
  ...
  const listings = useMemo(() => OFFICE_LISTINGS.filter(item => {
  ...
  const totalPages = Math.max(1, Math.ceil(listings.length / OFFICE_LISTINGS.length))
  const sizeOptions   = ['Any Size','0 – 100 sqm','100 – 300 sqm','300 – 800 sqm','800+ sqm']
  ...
  return (
    <div style={{ ...UB, backgroundColor:C.surface, color:C.ink, minHeight:'100vh' }}>
      <section style={{ position:'relative', height:520, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        <img src="https://images.pexels.com/photos/2062426/pexels-photo-2062426.jpeg?auto=compress&w=1600" alt="Offices" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }} />
        ...
        <div style={{ position:'relative', zIndex:10, textAlign:'center' as const, maxWidth:960, padding:'0 24px', width:'100%' }}>
          <h1 style={{ fontSize:'clamp(36px,6vw,64px)', ...UB, color:'white', marginBottom:36, lineHeight:1, textShadow:'0 4px 20px rgba(0,0,0,0.4)' }}>
            OFFICES & COMMERCIAL.<br/><span style={{ color:C.mint }}>BUILD YOUR BUSINESS.</span>
          </h1>
      ...
      <div style={{ maxWidth:1280, margin:'-40px auto 0', padding:'0 24px', position:'relative', zIndex:30 }}>
        <div style={{ backgroundColor:'rgba(255,255,255,0.97)', backdropFilter:'blur(16px)', border:'1px solid rgba(107,122,118,0.12)', borderRadius:100, boxShadow:'0 12px 40px rgba(0,0,0,0.08)', display:'flex', alignItems:'stretch', height:72 }}>
          ...
          <div style={{ width:1, backgroundColor:'rgba(107,122,118,0.12)', margin:'12px 0' }} />
          <DDrop label="SIZE (SQM)" value={beds} options={sizeOptions} open={bedsOpen} setOpen={setBedsOpen} onChange={setBeds} />
          <div style={{ width:1, backgroundColor:'rgba(107,122,118,0.12)', margin:'12px 0' }} />
      ...
      <main style={{ maxWidth:1280, margin:'0 auto', padding:'32px 24px 80px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:16, marginBottom:24, flexWrap:'wrap' as const }}>
          <div>
            <h2 style={{ fontSize:'clamp(20px,2.5vw,28px)', ...UB, color:C.ink, marginBottom:4 }}>Offices & Commercial for Sale in Morocco</h2>
            <p style={{ fontSize:14, color:C.mint, ...CB }}>2,380 Ads</p>
          </div>
          ...
          {ALL_CATS.map(cat=>(
            <Link key={cat.slug} href={`/${locale}/property/${cat.slug}`}
              style={{ padding:'10px 22px', borderRadius:100, fontSize:11, ...UB, textTransform:'uppercase' as const, letterSpacing:'0.1em', cursor:'pointer', border:'1px solid', textDecoration:'none', display:'inline-block',
                backgroundColor: cat.slug==='offices' ? C.mint : 'white', color: cat.slug==='offices' ? 'white' : C.muted, borderColor: cat.slug==='offices' ? C.mint : 'rgba(186,202,197,0.4)' }}>
              {cat.label}
            </Link>
          ))}
          ...
          <div style={{ display:'flex', flexDirection:'column' as const, gap:20 }}>
            ...
            <div style={{ background:'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', borderRadius:16, padding:'28px 32px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'relative', overflow:'hidden' }}>
              ...
              <div style={{ position:'relative', zIndex:1 }}>
                <h2 style={{ fontSize:20, fontWeight:800, color:'white', marginBottom:6 }}>What's your commercial property worth today?</h2>
            ...
            <div style={{ borderRadius:16, overflow:'hidden', border:'1px solid #f1f5f9' }}>
              <div style={{ background:'linear-gradient(135deg, #0f9b8e 0%, #22d4a8 100%)', padding:'28px 32px', display:'flex', alignItems:'center', gap:20 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize:13, color:'rgba(255,255,255,0.85)' }}>Explore commercial spaces with immersive 360° tours and AI-powered videos.</p>
                </div>
              </div>
            </div>
            <div style={{ position:'relative', borderRadius:16, overflow:'hidden', height:220, cursor:'pointer' }}>
              ...
              <div style={{ position:'absolute', inset:0, background:'linear-gradient(to right, rgba(15,23,42,0.92) 0%, rgba(15,23,42,0.62) 60%, transparent 100%)', display:'flex', flexDirection:'column' as const, justifyContent:'center', padding:32 }}>
                <div style={{ maxWidth:380 }}>
                  ...
                  <p style={{ fontSize:13, color:'#cbd5e1', marginBottom:18, lineHeight:1.6 }}>Boost your commercial listings to over 2M monthly seekers.</p>
          ...
          <aside style={{ display:'flex', flexDirection:'column' as const, gap:20, position:'sticky', top:114 }}>
            <div style={{ backgroundColor:'white', borderRadius:16, padding:20, border:'1px solid #f1f5f9', boxShadow:'0 4px 20px rgba(0,0,0,0.05)' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
                <h3 style={{ fontWeight:800, color:C.ink, fontSize:14 }}>New Commercial Developments</h3>
            ...
            <div style={{ background:'rgba(34,212,168,0.06)', border:'2px solid rgba(34,212,168,0.2)', borderRadius:16, padding:20 }}>
              ...
              <h3 style={{ fontSize:18, fontWeight:800, color:C.ink, marginTop:10, marginBottom:6 }}>List Your Commercial Space on SouKni</h3>
              <p style={{ fontSize:12, color:'#475569', marginBottom:14, lineHeight:1.6 }}>Reach over 2M business seekers monthly in Morocco.</p>
            ...
            <div style={{ backgroundColor:'white', borderRadius:16, padding:20, border:'1px solid #f1f5f9', boxShadow:'0 4px 20px rgba(0,0,0,0.05)' }}>
              ...
              <p style={{ fontSize:12, color:C.muted, marginBottom:14, lineHeight:1.6 }}>We'll notify you when new commercial spaces matching your search are listed.</p>
