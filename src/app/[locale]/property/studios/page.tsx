...
function PropertyCard({ prop }: { prop: typeof STUDIO_LISTINGS[0] }) {
...
export default function StudiosPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const listings = useMemo(() => STUDIO_LISTINGS.filter(item => {
  ...
  const totalPages = Math.max(1, Math.ceil(listings.length / STUDIO_LISTINGS.length))
