...
export default function TicketsCategoryPage() {
  ...
  useEffect(() => { setPage(1) }, [keyword, city, price, catSlug])
      ...
      <main style={{ maxWidth:'1280px', margin:'0 auto', padding:'32px 24px 80px' }}>
        <Breadcrumb
          items={[
            ...
            { label:'Tickets & Vouchers', href:`/${locale}/tickets-vouchers` },
            { label:catData.label },
          ]}
          mutedColor={C.muted}
          inkColor={C.ink}
        ...
        <CategoryFooterNav
          ...
          related={ALL_CATS.filter(c=>c.slug!==catSlug).slice(0,8).map(cat=>({ label:cat.label, href:`/${locale}/tickets-vouchers/${cat.slug}` }))}
          backHref={`/${locale}/tickets-vouchers`}
          backLabel="Back to All Tickets & Vouchers"
          inkColor={C.ink}
